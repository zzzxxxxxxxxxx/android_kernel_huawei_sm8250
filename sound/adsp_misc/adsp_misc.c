...
void adsp_misc_unmap_memory(void)
{
	if (g_adsp_misc_pdata == NULL) {
		hwlog_err("%s: adsp_misc in not initial\n", __func__);
		return;
	...
	if (IS_ERR_OR_NULL(g_adsp_misc_pdata->intf)) {
		hwlog_err("%s: intf ptr null or err\n", __func__);
		return;
	}

	if (!IS_ERR_OR_NULL(g_adsp_misc_pdata->intf->cal_unmap_memory)) {
		g_adsp_misc_pdata->intf->cal_unmap_memory();
		hwlog_info("%s: adsp_misc cal_unmap_memory\n", __func__);
	}

	return;

}
EXPORT_SYMBOL(adsp_misc_unmap_memory);
...
static int soc_adsp_send_karaoke_param(struct adsp_misc_ex_common_info *param,
	unsigned char *data, unsigned int len, struct adsp_misc_priv *pdata)
{
	int ret = 0;
	int dataType;
	...
	struct adsp_misc_ctl_info **param_in)
{
	...
	if (par_len < sizeof(struct adsp_misc_ctl_info)) {
		hwlog_err("%s: par_len is smaller than the struct size\n", __func__);
		return -EINVAL;
	}
#ifndef CONFIG_FINAL_RELEASE
...
static int soc_adsp_get_voice_inparam(struct adsp_ctl_param *adsp_param,
	struct adsp_misc_ex_common_info **param_in)
{
	unsigned int par_len;
	struct adsp_misc_ex_common_info *par_in = NULL;

	par_len = adsp_param->in_len;
	if (par_len < sizeof(struct adsp_misc_ex_common_info)) {
		hwlog_err("%s: par_len is smaller than the struct size\n", __func__);
		return -EINVAL;
	}
#ifndef CONFIG_FINAL_RELEASE
	hwlog_info("%s: param in len is %d, in_param is %p, struct size is %d",
		__func__, par_len, adsp_param->param_in, sizeof(struct adsp_misc_ex_common_info));
#endif
	...
	struct adsp_misc_ultrasonic_info **param_in)
{
	...
	par_len = adsp_param->in_len;
	if (par_len < sizeof(struct adsp_misc_ultrasonic_info)) {
		hwlog_err("%s: par_len is smaller than the struct size\n", __func__);
		return -EINVAL;
	}
#ifndef CONFIG_FINAL_RELEASE
...
{
	int ret;
	struct adsp_misc_ex_common_info *param_in = NULL;
	struct adsp_misc_data_pkg *result = NULL;
	struct adsp_ctl_param adsp_param;
...
#define HUAWEI_ECNS_MODULE_ID          0x11111200
#define VOICE_DIAGNOSE_MODE_PARAM_ID   0x11111204
static int soc_adsp_handle_voice_param(void __user *arg, int compat_mode,
	struct adsp_misc_priv *pdata)
{
	int ret;
	struct adsp_ctl_param adsp_param;
	...
	hwlog_info("%s: in_len is %d, out_len is %d\n", __func__, adsp_param.in_len, adsp_param.out_len);
	ret = soc_adsp_get_voice_inparam(&adsp_param, &param_in);
	if (ret < 0)
		goto err;
	...
	if ((param_out_len > MIN_PARAM_OUT) && (adsp_param.param_out != NULL)) {
		...
		module_id = HUAWEI_ECNS_MODULE_ID;
		param_id = 0x00010E21;
		result->size = param_out_len - sizeof(*result);
		if (g_adsp_misc_pdata->intf->voice_get_customize_param) {
			ret = g_adsp_misc_pdata->intf->voice_get_customize_param(module_id, param_id, result->size, result->data);
			if (ret < 0)
				goto err;
			...
			hwlog_err("%s:voice_get_customize_param function is NULL\n", __func__);
		}
	} else {
		module_id = HUAWEI_ECNS_MODULE_ID;
		param_id = VOICE_DIAGNOSE_MODE_PARAM_ID;
		hwlog_info("%s: in_size is %d, data_len is %d\n", __func__, param_in->param_size, param_in->size);
		if (g_adsp_misc_pdata->intf->voice_set_customize_param) {
			ret = g_adsp_misc_pdata->intf->voice_set_customize_param(module_id, param_id, param_in->size, param_in->data);
		} else {
			hwlog_err("%s:voice_set_customize_param function is NULL\n", __func__);
		}
	}
err:
	if (param_in != NULL)
			      ...
			      void __user *arg, int compat_mode)
{
	...
	case ADSP_MISC_IOCTL_KARAOKEMSG:
		...
		mutex_unlock(&g_adsp_misc_pdata->adsp_misc_mutex);
		break;
	case ADSP_MISC_IOCTL_VOICE_MSG:
		mutex_lock(&g_adsp_misc_pdata->adsp_misc_mutex);
		ret = soc_adsp_handle_voice_param(arg, compat_mode,
				g_adsp_misc_pdata);
		mutex_unlock(&g_adsp_misc_pdata->adsp_misc_mutex);
		break;
	default:
				   ...
				   unsigned long arg)
{
	...
	case ADSP_MISC_IOCTL_KARAOKEMSG_COMPAT:
		...
		break;
	case ADSP_MISC_IOCTL_VOICE_MSG_COMPAT:
		command = ADSP_MISC_IOCTL_VOICE_MSG;
	default:
		break;
	}
