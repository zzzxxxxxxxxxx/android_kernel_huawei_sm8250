...
static void complete_commit(struct msm_commit *c)
{
	...
	struct drm_bridge *bridge = priv->bridges[0];
	struct dsi_bridge *c_bridge = to_dsi_bridge(bridge);

	if (!c_bridge) {
		DRM_ERROR("%s: c_bridge is NULL!\n", __func__);
		return;
	}

	display = c_bridge->display;
	if (!display) {
		DSI_ERR("%s: display is NULL!\n", __func__);
		return;
	}
#endif
	...
	if (display->panel && (display->panel->dfps_caps.dfps_support)
		&& (display->panel->msm_atomic_fps_cmd)) {
		for_each_new_crtc_in_state(state, crtc, crtc_state, i);
		SDE_ATRACE_BEGIN("dsi_drm_send_dfps_cmd");
		dsi_drm_send_dfps_cmd(c_bridge, crtc_state);
		SDE_ATRACE_END("dsi_drm_send_dfps_cmd");
	}
#endif
