...
#define SWR_SSR_RESTER_WAIT_TIMER 1000
#define MAX_SWR_SSR_ERR_LIMIT 10

static int g_regmap_swr_read_err_count = 0;
			...
			void *val, size_t val_size)
{
	...
	if (ret < 0) {
		...
		g_regmap_swr_read_err_count++;
		if (g_regmap_swr_read_err_count >= MAX_SWR_SSR_ERR_LIMIT) {
			dev_err(dev, "%s:read_err_count exceed limit,restart adsp begin\n", __func__);
			subsystem_restart("adsp");
			dev_err(dev, "%s:read_err_count exceed limit,restart adsp end\n", __func__);
			msleep(SWR_SSR_RESTER_WAIT_TIMER);
			dev_err(dev, "%s: trigger SSR due to read_err_count is %d\n",
				__func__, g_regmap_swr_read_err_count);
			g_regmap_swr_read_err_count = 0;
		}
	} else {
		g_regmap_swr_read_err_count = 0;
	}
