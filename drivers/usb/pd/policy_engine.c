...
struct usbpd {
	...
	bool analog_headset_connected;
	struct work_struct hs_repoprt_work;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pd->analog_headset_connected = false;
#endif
		break;
...
static int psy_changed(struct notifier_block *nb, unsigned long evt, void *ptr)
{
	...
	if (pd->typec_mode == POWER_SUPPLY_TYPEC_NONE &&
		...
		pd->analog_headset_connected = true;
		queue_work(pd->wq, &pd->hs_repoprt_work);
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void report_hs_headset_status_work(struct work_struct *w)
{
	struct usbpd *pd = container_of(w, struct usbpd, hs_repoprt_work);

	usbpd_info(&pd->dev, "analog_headset_connected:%d\n",
		pd->analog_headset_connected);
	if (pd->analog_headset_connected)
		power_event_bnc_notify(POWER_BNT_HW_USB,
			POWER_NE_HW_USB_HEADPHONE, NULL);
	else
		power_event_bnc_notify(POWER_BNT_HW_USB,
			POWER_NE_HW_USB_HEADPHONE_OUT, NULL);
}
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	INIT_WORK(&pd->hs_repoprt_work, report_hs_headset_status_work);
#endif
