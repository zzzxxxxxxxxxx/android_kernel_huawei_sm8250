...
void hung_wp_screen_send_work(struct work_struct *work)
{
	...
	if (get_logusertype_flag() == BETA_USER)
		ret = snprintf_s(cmd_buf, COMAND_LEN_MAX, COMAND_LEN_MAX - 1,
			CMD_BUF_FMT_BETA, g_data.config->timeout +
			BUFFER_TIME_START, cur_stamp + BUFFER_TIME_END);
	else
		ret = snprintf_s(cmd_buf, COMAND_LEN_MAX, COMAND_LEN_MAX - 1,
			CMD_BUF_FMT, g_data.config->timeout + BUFFER_TIME_START,
	...
	zrhung_send_event(g_data.check_id, cmd_buf, "none");
...
void hung_wp_screen_start(int check_id)
{
	...
	pr_err("%s: going to check ID=%d timeout=%d\n",
	       __func__, check_id, g_data.config->timeout);
}
