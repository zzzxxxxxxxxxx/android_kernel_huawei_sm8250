...
#define RT8980_RESET_STATUS	0x21
...
/*
 * fsa4480_reset_event_occurs - check reset event occcur and config register back
 *
 * @node - phandle node to fsa4480 device
 *
 * Returns int to indicate any error
 */
int fsa4480_reset_event_occurs(struct device_node *node)
{
	struct i2c_client *client = of_find_i2c_device_by_node(node);
	struct fsa4480_priv *fsa_priv = NULL;
	unsigned int reset, status0, status1;
	int ret;
	...
	fsa_priv = (struct fsa4480_priv *)i2c_get_clientdata(client);
	if (!fsa_priv || !fsa_priv->regmap)
		return -EINVAL;

	/* Only RT8980 need to prevent the ESD reset */
	if (!fsa_priv->rt8980_used)
		return 0;

	ret = regmap_read(fsa_priv->regmap, RT8980_RESET_STATUS, &reset);
	ret |= regmap_read(fsa_priv->regmap, FSA4480_SWITCH_STATUS0, &status0);
	ret |= regmap_read(fsa_priv->regmap, FSA4480_SWITCH_STATUS1, &status1);
	if (ret)
		return ret;

	if ((reset & BIT(6)) || (status0 == 0x05 && status1 == 0x00)) {
		pr_info("rt8980 reset event triggerred\n");
		regcache_mark_dirty(fsa_priv->regmap);
		ret = regcache_sync(fsa_priv->regmap);
	}

	return 0;
}
EXPORT_SYMBOL(fsa4480_reset_event_occurs);
...
static bool fsa4480_is_writeable_reg(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case 0x01:
	case 0x04 ... 0x05:
...
static bool fsa4480_is_volatile_reg(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case 0x02 ... 0x03:
	case 0x06 ... 0x07:
...
static const struct regmap_config fsa4480_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
	.max_register = RT8980_RESET_STATUS,
	.cache_type = REGCACHE_FLAT,
	.num_reg_defaults_raw = (RT8980_RESET_STATUS + 1),
	.writeable_reg = fsa4480_is_writeable_reg,
	.volatile_reg = fsa4480_is_volatile_reg,
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	rc = regmap_read(fsa_priv->regmap, FSA4480_DEVICE_ID, &dev_id);
	if (rc)
		goto err_supply;

	if (dev_id & GENMASK(7, 6)) {
		/* Used to clear rt8980 reset event after POR */
		regmap_read(fsa_priv->regmap, RT8980_RESET_STATUS, &dev_id);
		fsa_priv->rt8980_used = true;
	}
