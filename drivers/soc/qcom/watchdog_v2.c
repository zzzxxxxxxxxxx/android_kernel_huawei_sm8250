...
#ifdef CONFIG_HW_SHUTDOWN_WDT
static bool g_pet_timer_enabled = true;
static unsigned long long g_pet_delay_time = 0;
...
#ifdef CONFIG_HW_SHUTDOWN_WDT
static bool msm_wdt_pet_stopped(struct msm_watchdog_data *wdog_dd)
{
	if (!g_pet_timer_enabled) {
...
#ifdef CONFIG_HW_SHUTDOWN_WDT
	if (msm_wdt_pet_stopped(wdog_dd)) {
		pr_err("wdt pet is stopped.\n");
		return;
	}
#endif
...
void msm_stop_pet_wdog(void)
{
	...
	pr_err("%s, pet_delay_time:%llu\n", __func__, g_pet_delay_time);
}
#endif
