...
static void dubai_copy_name(char *to, const char *from, size_t len)
{
	...
	if (strlen(from) <= len) {
		strncpy_s(to, len + 1, from, len);
		return;
	}

	p = strrchr(from, '/');
	if (p != NULL && (*(p + 1) != '\0'))
		strncpy_s(to, len + 1, p + 1, len);
	else
		strncpy_s(to, len + 1, from, len);
}
...
static void dubai_proc_entry_copy(unsigned char *value, const struct dubai_proc_entry *entry)
{
	...
	memset_s(&stat, sizeof(struct dubai_cputime), 0, sizeof(struct dubai_cputime));
	stat.uid = entry->uid;
	stat.pid = entry->tgid;
	...
	memcpy_s(value, sizeof(struct dubai_cputime), &stat, sizeof(struct dubai_cputime));
}
...
static void dubai_free_proc_entry(struct dubai_proc_entry *entry)
{
	...
	memset_s(entry, sizeof(struct dubai_proc_entry), 0, sizeof(struct dubai_proc_entry));
...
static void dubai_free_thread_entry(struct dubai_thread_entry *entry)
{
	...
	memset_s(entry, sizeof(struct dubai_thread_entry), 0, sizeof(struct dubai_thread_entry));
...
update_thread_name:
	snprintf_s(thread->name, NAME_LEN, NAME_LEN - 1, "%s_%s", entry->name, comm);
...
static struct dubai_proc_entry *dubai_register_proc_entry_locked(uid_t uid, pid_t tgid, bool cmdline, const char *name)
{
	...
	strncpy_s(entry->name, NAME_LEN, name, NAME_LEN - 1);
	entry->threads = NULL;
...
static void dubai_proc_cputime_reset(void)
{
	...
	memset_s(proc_entry_avail_pool, sizeof(proc_entry_avail_pool), 0, sizeof(proc_entry_avail_pool));
	memset_s(thread_entry_avail_pool, sizeof(thread_entry_avail_pool), 0, sizeof(thread_entry_avail_pool));
	mutex_unlock(&dubai_proc_lock);
