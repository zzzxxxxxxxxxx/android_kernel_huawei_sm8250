...
static void dubai_get_kworker_stats(struct buffered_log_entry *log_entry, long long timestamp, int max_count)
{
	...
	hash_for_each_safe(kworker_hash_table, bkt, tmp, entry, hash) {
		...
		if (strlen(entry->info.symbol) == 0) {
			...
			strncpy_s(entry->info.symbol, MAX_SYMBOL_LEN, buffer, MAX_SYMBOL_LEN - 1);
		}

		if (transmit->count < max_count) {
			memcpy_s(data, sizeof(struct dubai_kworker_info), &entry->info, sizeof(struct dubai_kworker_info));
...
static struct dubai_uevent_entry *dubai_find_uevent_entry(const char *devpath)
{
	...
	strncpy_s(entry->uevent.devpath, MAX_DEVPATH_LEN, devpath, MAX_DEVPATH_LEN - 1);
...
static void dubai_get_uevent_stats(struct buffered_log_entry *log_entry, long long timestamp, int max_count)
{
	...
	list_for_each_entry_safe(entry, tmp, &uevent_list, list) {
		...
		if (transmit->count < max_count) {
			memcpy_s(data, sizeof(struct dubai_uevent_info), &entry->uevent, sizeof(struct dubai_uevent_info));
			data += sizeof(struct dubai_uevent_info);
			transmit->count++;
		}
		memset_s(&(entry->uevent.actions), KOBJ_MAX * sizeof(int), 0, KOBJ_MAX * sizeof(int));
	}
	mutex_unlock(&uevent_lock);
}
	...
	const struct dubai_binder_proc *proc)
{
	...
	if (!entry) {
		...
		memset_s(&entry->proc, sizeof(struct dubai_binder_proc), 0, sizeof(struct dubai_binder_proc));
		entry->count = 0;
...
static int dubai_get_binder_proc_cmdline(struct dubai_binder_proc *service, struct dubai_binder_proc *client)
{
	...
	if (!stats) {
		...
		strncpy_s(service->cmdline, CMDLINE_LEN, service_cmdline, CMDLINE_LEN - 1);
		strncpy_s(client->cmdline, CMDLINE_LEN, client_cmdline, CMDLINE_LEN - 1);
	} else {
		strncpy_s(service->cmdline, CMDLINE_LEN, stats->proc.cmdline, CMDLINE_LEN - 1);
		entry = dubai_check_binder_client_entry(&stats->client, client);
		if (!entry) {
			mutex_unlock(&binder_stats_hash_lock);
			memset_s(client_cmdline, CMDLINE_LEN, 0, CMDLINE_LEN);
			rc = dubai_get_cmdline_by_uid_pid(client->uid, client->pid, client_cmdline, CMDLINE_LEN);
			if (rc < 0) {
				dubai_err("Failed to get client cmdline: %d, %d", client->uid, client->pid);
				return -1;
			...
			strncpy_s(client->cmdline, CMDLINE_LEN, entry->proc.cmdline, CMDLINE_LEN - 1);
			mutex_unlock(&binder_stats_hash_lock);
		}
	}
	...
	struct dubai_binder_stats_transmit *transmit, int count)
{
	...
	list_for_each_entry_safe(client, temp, &stats->client, node) {
		...
		if (transmit->count < count) {
			memcpy_s(data, sizeof(struct dubai_binder_stats_info), &info, sizeof(struct dubai_binder_stats_info));
...
void dubai_log_binder_stats(int reply, uid_t c_uid, int c_pid, uid_t s_uid, int s_pid)
{
	...
	memset_s(&service, sizeof(struct dubai_binder_proc), 0, sizeof(struct dubai_binder_proc));
	service.uid = s_uid;
	service.pid = s_pid;
	memset_s(&client, sizeof(struct dubai_binder_proc), 0, sizeof(struct dubai_binder_proc));
	client.uid = c_uid;
	client.pid = c_pid;
