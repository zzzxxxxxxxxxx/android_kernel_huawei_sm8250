...
static int dubai_peri_get_volt_time(void __user *argp)
{
	...
	memcpy_s(last_volt_times, sizeof(struct dubai_peri_volt_time) * PERI_VOLT_CNT_MAX,
		curr_volt_times, sizeof(struct dubai_peri_volt_time) * PERI_VOLT_CNT_MAX);
	memset_s(curr_volt_times, sizeof(curr_volt_times), 0, sizeof(curr_volt_times));
...
static int dubai_peri_get_ip_stats(void __user *argp)
{
	...
	for (i = 0; i < nr_ip; i++) {
		strncpy_s(delta->ip, PERI_IP_NAME_LEN, curr_ip_stats[i].ip, PERI_IP_NAME_LEN - 1);
		for (j = 0; j < nr_volt; j++) {
	...
	memcpy_s(last_ip_stats, sizeof(struct dubai_peri_ip_stats) * PERI_IP_CNT_MAX,
		curr_ip_stats, sizeof(struct dubai_peri_ip_stats) * PERI_IP_CNT_MAX);
	memset_s(curr_ip_stats, sizeof(curr_ip_stats), 0, sizeof(curr_ip_stats));
...
{
	memset_s(curr_volt_times, sizeof(curr_volt_times), 0, sizeof(curr_volt_times));
	memset_s(last_volt_times, sizeof(last_volt_times), 0, sizeof(last_volt_times));
	memset_s(curr_ip_stats, sizeof(curr_ip_stats), 0, sizeof(curr_ip_stats));
	memset_s(last_ip_stats, sizeof(last_ip_stats), 0, sizeof(last_ip_stats));
}
