...
static int dubai_ddr_get_time_in_state(void __user *argp)
{
	...
	memcpy_s(&last_time_in_state, sizeof(struct dubai_ddr_time_in_state), &curr_time_in_state, sizeof(struct dubai_ddr_time_in_state));
	memset_s(&curr_time_in_state, sizeof(curr_time_in_state), 0, sizeof(curr_time_in_state));
...
static int dubai_ddr_get_ip_stats(void __user *argp)
{
	...
	for (i = 0; i < nr_ip; i++) {
		strncpy_s(delta->ip, DDR_IP_NAME_LEN, curr_ip_stats[i].ip, DDR_IP_NAME_LEN - 1);
		for (j = 0; j < nr_freq; j++) {
	...
	memcpy_s(last_ip_stats, sizeof(struct dubai_ddr_ip_stats) * DDR_IP_CNT_MAX,
		curr_ip_stats, sizeof(struct dubai_ddr_ip_stats) * DDR_IP_CNT_MAX);
...
{
	memset_s(&curr_time_in_state, sizeof(curr_time_in_state), 0, sizeof(curr_time_in_state));
	memset_s(&last_time_in_state, sizeof(last_time_in_state), 0, sizeof(last_time_in_state));
	memset_s(curr_ip_stats, sizeof(curr_ip_stats), 0, sizeof(curr_ip_stats));
