...
void dubai_wakeup_notify(const char *source)
{
	...
	memset_s(last_wakeup_source, WAKEUP_SOURCE_SIZE * sizeof(unsigned char),
		0, WAKEUP_SOURCE_SIZE * sizeof(unsigned char));
	strncpy_s(last_wakeup_source, WAKEUP_SOURCE_SIZE, source, WAKEUP_SOURCE_SIZE - 1);
...
void dubai_update_suspend_abort_reason(const char *reason)
{
	...
	strncat_s(suspend_abort_reason, MAX_SUSPEND_ABORT_LEN, reason, new_reason_len);
...
void dubai_set_rtc_timer(const char *name, int pid)
{
	...
	memcpy_s(rtc_timer_info.name, TASK_COMM_LEN - 1, name, TASK_COMM_LEN - 1);
...
static void dubai_send_wakeup_stats(void)
{
	...
	kfree(wakeup_info);
	memset_s(&rtc_timer_info, sizeof(struct dubai_rtc_timer_info), 0, sizeof(struct dubai_rtc_timer_info));
}
...
static void dubai_send_suspend_abort_reason(void)
{
	...
	memset_s(suspend_abort_reason, MAX_SUSPEND_ABORT_LEN, 0, MAX_SUSPEND_ABORT_LEN);
}
