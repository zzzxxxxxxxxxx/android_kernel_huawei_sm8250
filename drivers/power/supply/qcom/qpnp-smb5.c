...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->usb_online_active = of_property_read_bool(node, "usb-online-active");
	chg->support_pd_to_scp = of_property_read_bool(node, "support_pd_to_scp");
	chg->need_wait_psy = of_property_read_bool(node, "need-wait-psy");
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
static void smb5_set_real_type_prop(struct smb_charger *chg, int charger_type)
{
	...
	chg->real_charger_type = real_charger_type;
	if (chg->usb_third_bc_12 || chg->real_charger_type == POWER_SUPPLY_TYPE_FCP)
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	case POWER_SUPPLY_PROP_AICL_THRESHOLD:
		if (chg->support_pd_to_scp) {
			rc = smblib_write(chg, USBIN_CONT_AICL_THRESHOLD_REG, val->intval);
			if (rc < 0) {
				pr_err("Couldn't set aicl threshold to %d, rc=%d\n", val->intval, rc);
				rc = -EINVAL;
			}
		}
		break;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static int smb5_configure_iterm_thresholds(struct smb5 *);
static int smb5_set_iterm_thresholds(struct smb_charger *chg,
		const union power_supply_propval *val)
{
	int rc;
	struct smb5 *chip = container_of(chg, struct smb5, chg);
	chip->dt.term_current_thresh_hi_ma = -val->intval;
	rc = smb5_configure_iterm_thresholds(chip);
	if (rc < 0) {
		pr_err("%s:Could't configure ITERM thresholds", __func__);
		return rc;
	}
	pr_info("%s update iterm to %d\n", __func__, val->intval);
	return rc;
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		rc = smb5_set_iterm_thresholds(chg, val);
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	default:
		rc = -EINVAL;
