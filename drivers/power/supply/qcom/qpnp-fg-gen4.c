...
struct fg_gen4_chip {
	...
	bool			curr_avg_en;
	struct delayed_work	current_update_work;
	int			filter_count;
	int			filter_sum;
	int			filter_len;
	int			curr_filter[MAX_WINDOW_LEN];
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define BATT_HIGH_WARNING_TEMP     680
#define BATT_WARNING_TEMP_READ_CNT 3
#define BATT_TEMP_MAX_FG_IR        55
void fg_temperature_filter(struct fg_dev *fg, int *val)
{
	int i;
	int rc = 0;
	...
	if (chip->batt_therm_chan) {
		for (i = 0; i < BATT_WARNING_TEMP_READ_CNT; i++) {
			rc = iio_read_channel_processed(chip->batt_therm_chan, val);
			if (rc < 0) {
				pr_err("Failed reading BAT_TEMP over ADC rc=%d\n", rc);
				return;
			}
			pr_err("[%s]temp too high cnt = %d, battery temperature is = %d\n",
				__func__, i, *val);
			batt_temperature = batt_temperature + *val;
			msleep(10);
		}
		*val = batt_temperature / BATT_WARNING_TEMP_READ_CNT;
	}
}
...
static int fg_gen4_get_battery_temp(struct fg_dev *fg, int *val)
{
	...
	if (chip->batt_therm_chan) {
		...
		if (chip->dt.soc_temp_filt)
			if (*val >= (BATT_HIGH_WARNING_TEMP - BATT_TEMP_MAX_FG_IR))
				fg_temperature_filter(fg, val);
	} else {
...
static int fg_gen4_get_prop_capacity(struct fg_dev *fg, int *val)
{
	...
	if (chip->vbatt_low) {
		...
		if (rc < 0) {
			pr_err("Error in getting bat vol when fg_gen4_get_prop_capacity, rc=%d\n", rc);
			return 0;
		}

		vbatt_mv /= 1000;
		if (vbatt_mv > chip->dt.cutoff_volt_mv) {
			pr_info("set vbatt_low false fg_gen4_get_prop_capacity\n");
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void fg_reset_curr_fifo(struct fg_gen4_chip *chip)
{
	struct fg_dev *fg = &chip->fg;
	unsigned int i;
	int rc;
	...
	rc = fg_get_battery_current(fg, &curr);
	if (rc < 0)
		pr_err("Failed to get battery current rc=%d\n", rc);

	chip->filter_sum = 0;
	for (i = 0; i < chip->filter_len; i++) {
		chip->curr_filter[i] = curr;
		chip->filter_sum += curr;
	}
}

static void fg_update_curr_fifo(struct fg_gen4_chip *chip)
{
	struct fg_dev *fg = &chip->fg;
	int index;
	int rc;
	...
	rc = fg_get_battery_current(fg, &curr);
	if (rc < 0) {
		pr_err("Failed to update current fifo\n");
		return;
	}

	index = chip->filter_count % chip->filter_len;
	chip->filter_sum -= chip->curr_filter[index];
	chip->curr_filter[index] = curr;
	chip->filter_sum += chip->curr_filter[index];
	if (++chip->filter_count >= chip->filter_len)
		chip->filter_count = 0;
}

static void current_update_work(struct work_struct *work)
{
	struct fg_gen4_chip *chip = container_of(work, struct fg_gen4_chip,
		current_update_work.work);
	if (!chip)
		return;
	fg_update_curr_fifo(chip);

	schedule_delayed_work(&chip->current_update_work,
		msecs_to_jiffies(chip->sampling_interval));
}

static int fg_get_battery_current_avg(struct fg_gen4_chip *chip, int *val)
{
	if (!chip || (chip->filter_len == 0))
		return -EINVAL;
	*val = chip->filter_sum / chip->filter_len;
	return 0;
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		if (chip->curr_avg_en)
			rc = fg_get_battery_current_avg(chip, &pval->intval);
                else
...
static int fg_gen4_hw_init(struct fg_gen4_chip *chip)
{
	...
	if (chip->dt.soc_scale_mode) {
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		rc = fg_gen4_set_vbatt_tau(chip,
			(chip->dt.soc_scale_quirk ? VBATT_TAU_CUSTOMIZE : VBATT_TAU_DEFAULT));
#else
...
static int fg_gen4_parse_dt(struct fg_gen4_chip *chip)
{
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	chip->curr_avg_en = of_property_read_bool(node, "qcom,curr-avg-en");
	if (chip->curr_avg_en) {
		chip->filter_len = MAX_WINDOW_LEN;
		of_property_read_u32(node, "qcom,filter-len", &chip->filter_len);
		if ((chip->filter_len < 1) || (chip->filter_len > MAX_WINDOW_LEN))
			chip->filter_len = MAX_WINDOW_LEN;

		chip->sampling_interval = DEFAULT_SAMPLING_INTERVAL_MS;
		of_property_read_u32(node, "qcom,sampling-interval",
			&chip->sampling_interval);
		if (chip->sampling_interval <= 0)
			chip->sampling_interval = DEFAULT_SAMPLING_INTERVAL_MS;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	...
	chip->dt.soc_scale_quirk = of_property_read_bool(node, "qcom,soc-scale-quirk");
	chip->dt.soc_temp_filt = of_property_read_bool(node, "soc-temp-filt");
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	return 0;
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chip->curr_avg_en)
		cancel_delayed_work_sync(&chip->current_update_work);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chip->curr_avg_en) {
		fg_reset_curr_fifo(chip);
		INIT_DELAYED_WORK(&chip->current_update_work, current_update_work);
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chip->curr_avg_en)
		schedule_delayed_work(&chip->current_update_work, 0);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
