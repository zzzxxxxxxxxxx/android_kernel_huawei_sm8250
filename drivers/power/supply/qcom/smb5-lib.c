...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_FCP && !chg->usb_third_bc_12)
		result = &smblib_apsd_results[FCP];
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_DCP && !chg->usb_third_bc_12)
			       ...
			       union power_supply_propval *val)
{
	...
	if ((stage == DC_STAGE_DEFAULT) &&
		(get_client_vote_locked(chg->usb_icl_votable, USER_VOTER) == 0)) {
		val->intval = false;
		return rc;
	}
#else
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->flash_active || chg->usb_online_active)
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	dev_info(chg->dev, "pd active %d,val %d\n", chg->pd_active, val->intval);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	chg->pd_active = val->intval;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		dev_info(chg->dev, "vote dis PD_SUSPEND\n");
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define CHARGE_RELEASE_LOCK_DELAY_MS	60000 /* 1 min */
static void smblib_charge_release_lock_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						charge_release_lock_work.work);
	int rc;
	u8 stat;

	pr_info("%s trigger release lock work\n", __func__);
	rc = smblib_read(chg, BATTERY_CHARGER_STATUS_1_REG, &stat);
	if (rc < 0) {
		pr_err("%s Couldn't read BATTERY_CHARGER_STATUS_1 rc=%d\n", __func__, rc);
		return;
	}

	if ((stat & BATTERY_CHARGER_STATUS_MASK) == TERMINATE_CHARGE) {
		power_wakeup_unlock(chg->plugin_wakelock, false);
		huawei_battery_handle_charge_done();
		pr_info("%s release charge lock ok\n", __func__);
	}
	return;
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
irqreturn_t chg_state_change_irq_handler(int irq, void *data)
{
	...
	if (stat == TERMINATE_CHARGE)
		schedule_delayed_work(&chg->charge_release_lock_work,
				msecs_to_jiffies(CHARGE_RELEASE_LOCK_DELAY_MS));
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		power_wakeup_lock(chg->plugin_wakelock, false);
		if (chg->usb_third_bc_12 &&
			(chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS))
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		pd_dpm_set_pd_charger_status(&ctc_flag);
		if (!chg->usb_third_bc_12)
			smblib_apsd_enable(chg, true);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS) {
		if (chg->need_wait_psy)
			schedule_work(&chg->wait_psy_work);
		else
			power_supply_set_int_property_value("huawei_charge",
				POWER_SUPPLY_PROP_CHG_PLUGIN, vbus_rising);
	}
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	if (typec_rp_med_high(chg, typec_mode) && !chg->usb_third_bc_12 && (pst != POWER_SUPPLY_TYPE_FCP)) {
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (chg->pd_active && !chg->support_pd_to_scp)
		return IRQ_HANDLED;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define WAIT_PSY_REG_DELAY_MS 200
#define CYCLE 10
static void smblib_wait_psy_work(struct work_struct *work)
{
	int rc;
	int count;
	u8 stat;
	...
	struct smb_charger *chg = container_of(work, struct smb_charger, wait_psy_work);
	for (count = 0; count < CYCLE; count++) {
		if (power_supply_get_by_name("huawei_charge")) {
			rc = smblib_read(chg, USBIN_BASE + INT_RT_STS_OFFSET, &stat);
			if (rc < 0) {
				smblib_err(chg, "Couldn't read USB_INT_RT_STS rc=%d\n", rc);
				return;
			...
			msleep (WAIT_PSY_REG_DELAY_MS);
			count++;
		}
	}
	smblib_err(chg, "get huawei_charge psy name fail\n");
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_WORK(&chg->wait_psy_work, smblib_wait_psy_work);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	INIT_DELAYED_WORK(&chg->charge_release_lock_work, smblib_charge_release_lock_work);
	chg->plugin_wakelock = power_wakeup_source_register(chg->dev,
					"smblib_plugin_wakelock");
	if (!chg->plugin_wakelock)
		smblib_err(chg, "register smblib_plugin_wakelock failed\n");
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_delayed_work_sync(&chg->pr_swap_detach_work);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		cancel_delayed_work_sync(&chg->charge_release_lock_work);
		power_wakeup_source_unregister(chg->plugin_wakelock);
		cancel_delayed_work_sync(&chg->period_update_work);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
