...
#if IS_ENABLED(CONFIG_QTI_PMIC_GLINK)
static void huawei_charger_send_charging_event(int val)
{
	int usb_in = 0;
	int wls_in = 0;

	(void)power_supply_get_int_property_value("usb", POWER_SUPPLY_PROP_ONLINE, &usb_in);
	(void)power_supply_get_int_property_value("Wireless", POWER_SUPPLY_PROP_ONLINE, &wls_in);

	pr_info("%s usb_in=%d, wls_in=%d enable=%d\n", __func__, usb_in, wls_in, val);
	if (usb_in || wls_in) {
		if (val)
...
static int huawei_charger_set_rt_current(struct charge_device_info *di, int val)
{
	...
	power_vote_set(USB_ICL_VOTE_OBJECT, CHARGE_RT_VOTER, true, iin_rt_curr);
...
{
	int ret;
	int iin = (value > 0) ? value : DCP_INPUT_CURRENT;

	ret = power_vote_set(USB_ICL_VOTE_OBJECT, CHARGE_THERMAL_VOTER, true, iin);
	if (ret) {
		hwlog_err("%s vote fail, disable voter\n", __func__, ret);
		power_vote_set(USB_ICL_VOTE_OBJECT, CHARGE_THERMAL_VOTER, false, 0);
        }
...
void huawei_charger_set_fcp_current_limit(unsigned int value)
{
	...
	if (fcp_pd_support_icl == 1) {
		power_vote_set(USB_ICL_VOTE_OBJECT, CHARGE_FCP_VOTER, true, value);
...
static int set_thirdparty_buck_rt_current(struct charge_device_info *di, int val)
{
	int iin_rt_curr;
	...
	if (val == 0) {
		charge_set_input_current_max();
		iin_rt_curr = charge_get_input_current_max();
	} else if ((val <= MIN_CURRENT) && (val > 0)) {
	...
	charge_set_input_current(iin_rt_curr);
	charge_set_iin_rt_curr(iin_rt_curr);
	di->sysfs_data.iin_rt_curr = iin_rt_curr;
	return 0;
}
...
static int huawei_charger_set_rt_current(struct charge_device_info *di, int val)
{
	...
	if (charge_support_thirdparty_buck())
		return set_thirdparty_buck_rt_current(di, val);
...
static int huawei_charger_get_mains_online(void)
{
	int usb_online = 0;
	unsigned int type;
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	case CHARGE_SYSFS_ADAPTOR_VOLTAGE:
		if (charge_get_reset_adapter_source())
			...
			return snprintf(buf, PAGE_SIZE, "%d\n", ADAPTER_9V);
		break;
	case CHARGE_SYSFS_MAINS_ONLINE:
		return snprintf(buf, PAGE_SIZE, "%d\n", huawei_charger_get_mains_online());
					...
					const char *buf, size_t count)
{
	...
	case CHARGE_SYSFS_FACTORY_DIAG_CHARGER:
		...
		if (power_glink_set_property_value(POWER_GLINK_PROP_ID_SET_CHARGE_ENABLE, (unsigned int *)&val, GLINK_DATA_ONE))
			return -EFAULT;
		huawei_charger_send_charging_event(val);
#else
		if (!power_supply_check_psy_available("battery", &di->batt_psy))
			return -EFAULT;
		power_supply_set_int_property_value_with_psy(di->batt_psy,
...
static int fcp_set_enable_charger(unsigned int val)
{
	...
	hwlog_info("fcp: ENABLE_CHARGER set %d\n", val);
	return 0;
}
...
static int dcp_set_iin_limit_array(unsigned int idx, unsigned int val)
{
	dcp_set_iin_thermal_limit(DCP_INPUT_CURRENT);
	return 0;
}
...
{
	int cur = (val > 0) ? val : DCP_IBAT_CURRENT;

	return power_vote_set(FCC_VOTE_OBJECT, CHARGE_THERMAL_VOTER, true, cur);
}
#else
static int dcp_set_vterm_dec(unsigned int val)
{
