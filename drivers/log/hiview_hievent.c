...
int hiview_hisysevent_set_pid(struct hiview_hisysevent *event)
{
	...
	event->pid = (unsigned int)(current->pid);
	return 0;
}
...
int hiview_hisysevent_set_tid(struct hiview_hisysevent *event)
{
	...
	event->tid = (unsigned int)(current->tgid);
	return 0;
}
...
int hiview_send_hisysevent_init(void)
{
	...
	addrlen = (int)(strlen(HISYSEVENT_WRITER_DEV) + offsetof(struct sockaddr_un, sun_path) + SYS_SIZE_OFFSET_1);
