...
/*
 * Description : write diag log info
 */
int rdr_write_diaginfo_file(char *buf, size_t len)
{
	char *path = NULL;

	path = rdr_generate_path(g_file_device->root_path, "diaginfo.log", NULL);
	if (path == NULL) {
		PRINT_ERROR("%s(): create path failed", __func__);
		return -1;
	}
	if (rdr_file_append(path, buf, len) != 0)
		PRINT_ERROR("%s(): write diaginfo file failed", __func__);
	kfree(path);
	return 0;
}
