...
/*
 * Description : General API of file write append
 */
int rdr_file_append(const char *file, char *tmp_buf, size_t len)
{
	int ret;
	int fd_dfx;

	fd_dfx = SYS_OPEN(file, O_CREAT | O_WRONLY | O_APPEND, FILE_LIMIT);
	if (fd_dfx < 0) {
		PRINT_ERROR("%s():%d:open fail[%d]\n", __func__, __LINE__, fd_dfx);
		return -1;
	}
