	...
	struct device_node *np, struct dtsi_attr_req_msg *msg)
{
	...
	case DTSI_CMD_READ_U32:
		...
		if (ret) {
			g_get_prop_errno = PROP_NOT_EXIST;
	...
	case DTSI_CMD_READ_STRING:
		...
		break;
	default:
		g_get_prop_errno = IVALID_DTSI_CMD;
		goto ERROR;
	}
	return;

ERROR:
	cust_attr_send_err(client, g_get_prop_errno);
}
	...
	struct dtsi_attr_req_msg *msg)
{
	...
	if (!np) {
		g_get_prop_errno = NODE_NOT_FOUND;
		hwlog_err("%s: not found dts node\n", __func__);
		goto ERROR;
	}

	if (!of_device_is_available(np)) {
		g_get_prop_errno = DEVICE_NOT_VALID;
