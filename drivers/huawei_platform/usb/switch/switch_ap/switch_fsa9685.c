...
int rt8979_psy_chg_type_changed(struct fsa9685_device_info *di)
{
	...
	if (g_usb_sw_oe_gpio != -1) {
		/* To avoid noise affect MTP transfer, close usb switch when recognize the usb. */
		if (di->chg_type == CHARGER_TYPE_USB || di->chg_type == CHARGER_TYPE_BC_USB) {
			gpio_set_value(g_usb_sw_oe_gpio, 1);
			hwlog_info("set g_usb_sw_oe_gpio high\n");
		} else {
			gpio_set_value(g_usb_sw_oe_gpio, 0);
			hwlog_info("set g_usb_sw_oe_gpio low\n");
		}
	}
...
static int fsa9685_parse_dts(struct device_node* np, struct fsa9685_device_info *di)
{
	...
	if (power_gpio_config_output(np, "usb_sw_oe_gpio", "usb_sw_oe_gpio",
		&g_usb_sw_oe_gpio, 0)) {
		hwlog_err("config g_usb_sw_oe_gpio fail\n");
		g_usb_sw_oe_gpio = -1;
	}
