...
int batt_uuid_read_cb(unsigned char version, void *data, int len)
{
	if (!data || (len != sizeof(struct batt_uuid_bind))) {
		hwlog_err("[%s] battery bind data error length is %d\n", __func__, len);
		return -1;
	}

	hwlog_info("[%s] read battery uuid bind info success\n", __func__);
...
int batt_uuid_write_cb(unsigned char version, void *data, int len)
{
	...
	hwlog_info("[%s] write battery uuid bind info success\n", __func__);
	mutex_lock(&g_bind_info.lock);
	/* clear update sign */
...
void batt_write_bind_mesg(void)
{
	...
	for (i = 0; i < POWER_MESG_RETRY_TIMES; i++) {
		hwlog_info("[%s] send write battery bind %d mesg\n", __func__, i);
		mutex_lock(&g_bind_info.lock);
		...
		mutex_unlock(&g_bind_info.lock);
		if (ret) {
			hwlog_err("[%s] write bind power mesg send failed\n", __func__);
			return;
		}
...
int batt_get_bind_uuid(struct batt_uuid_bind *info)
{
	...
	hwlog_info("[%s] send read bind mesg\n", __func__);
	batt_binder_init();
