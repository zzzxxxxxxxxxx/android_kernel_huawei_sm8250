...
void bsc_key_func(struct work_struct *work)
{
	...
	hwlog_info("[%s] checker no:%d group:%d prepare to certification\n",
...
sn_func_entry:
	...
	if (res.len > 0) {
		...
		if (send_batt_info_mesg(&info, (void *)res.data, res.len)) {
			hwlog_err("[%s] checker no:%d group:%d send attr fail\n",
				__func__, drv_data->id_in_grp, drv_data->id_of_grp);
	...
	if (bco->get_batt_sn(drv_data, &drv_data->sn_res, &drv_data->sn,
		&drv_data->sn_len)) {
		hwlog_err("[%s] battery no:%d group:%d get batt sn failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		sn_fail++;
		hwlog_err("[%s] get batt sn fail %d@%d\n", __func__, sn_fail,
			BATT_SN_READ_RETRY);
	...
	if (legal_sn_check_0(drv_data)) {
		hwlog_err("[%s] battery no:%d group:%d sn_checker failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		return;
	}
}
...
static void cycles_work_func(struct work_struct *work)
{
	...
	if ((drv_data->batt_rematch_current == BATTERY_REMATCHABLE) &&
		...
		if (ret)
			hwlog_err("[%s] set battery old failed\n",
				__func__);
		else
...
int bsc_register_cycles_listener(struct batt_chk_data *drv_data)
{
	...
	ret = drv_data->bco.get_batt_safe_info(drv_data, BATT_MATCH_ABILITY,
		&drv_data->batt_rematch_onboot);
	if (ret) {
		hwlog_err("[%s] get battery matchable status failed\n", __func__);
