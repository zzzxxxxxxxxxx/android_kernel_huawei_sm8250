...
static int fcp_charge_handle(struct huawei_battery_info *info)
{
	int ret;
	int i;
	...
	/* reset adapter and try max 3 times */
	for (i = 0; i < 3 && ret == 1; i++) {
...
void fcp_charge_check(struct huawei_battery_info *info)
{
	...
	if ((info->enable_hv_charging == false) && (charge_get_first_insert() == 1)) {
