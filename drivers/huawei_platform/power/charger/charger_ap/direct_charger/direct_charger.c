...
int direct_charge_get_iterm(unsigned int *val)
{
	...
	if (l_di->stage_size > 0)
		*val = l_di->orig_volt_para[l_di->stage_size - 1].cur_th_low;
...
int direct_charge_init_adapter_and_device(int flag)
{
	...
	if (!(dcm_use_two_stage() && (l_di->working_mode == SC4_MODE))) {
		ret = dc_init_adapter(5000); /* 5000:adapter init vset */
		if (ret) {
...
int direct_charge_open_charging_path(void)
{
	...
	l_di->adaptor_iset = CURRENT_SET_FOR_RES_DETECT + l_di->init_delt_iset + dc_get_gain_ibus();
	direct_charge_set_adapter_voltage_step_by_step(l_di->adaptor_vset);
	dc_set_adapter_current(l_di->adaptor_iset);
	if (!direct_charge_check_enable_status() || dc_get_stop_charging_flag())
		return -1;
...
fail_open:
	...
	dc_show_realtime_charging_info(l_di->working_mode, l_di->cur_mode);
...
static int direct_charge_select_adp_max_curr(struct direct_charge_device *di)
{
	...
	ilimit = dc_get_adapter_ilimit();
	if (ilimit)
		adp_max_cur = adp_max_cur > ilimit ? ilimit : adp_max_cur;
...
static bool direct_charge_mode_judge(int mode, unsigned int orientation)
{
	...
	switch (orientation) {
	case UPWARD:
		if (!other->pri_inversion || (other->sysfs_iin_thermal <= other->cur_inversion) ||
			(other->sysfs_iin_thermal <= l_di->cur_ibat_th_high))
			return false;
	...
	case DOWNWARD:
		if ((l_di->sysfs_iin_thermal == l_di->iin_thermal_default) || (l_di->sysfs_iin_thermal == 0) ||
			(l_di->sysfs_iin_thermal > l_di->cur_inversion) ||
			(l_di->sysfs_iin_thermal > l_di->cur_ibat_th_high) ||
			(l_di->max_adaptor_cur && (l_di->sysfs_iin_thermal > (l_di->max_adaptor_cur * l_di->dc_volt_ratio))))
			return false;
...
void direct_charge_stop_charging(void)
{
	...
	dc_set_adapter_voltage(ADAPTER_5V * POWER_MV_PER_V);
