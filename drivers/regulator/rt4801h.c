...
/*
static void rt4801h_disable(){
	gpiod_set_value_cansleep(gpio_to_desc(gpio_vsp_enable), VSP_DISABLE);
	mdelay(10);
	gpiod_set_value_cansleep(gpio_to_desc(gpio_vsn_enable), VSN_DISABLE);
...
static int rt4801h_finish_setting(void)
{
	...
	retval = gpio_direction_output(rt4801h_client->config.gpio_vsn_enable, VSN_DISABLE);
	if (retval != 0) {
		pr_err("failed to output gpio %d : gpio_lcd_n5v5_enable !\n", rt4801h_client->config.gpio_vsn_enable);
		return retval;
	}
	/* delay 10us */
	udelay(10);

	retval = gpio_direction_output(rt4801h_client->config.gpio_vsp_enable, VSP_DISABLE);
	if (retval != 0) {
		pr_err("failed to output gpio %d : gpio_lcd_p5v5_enable !\n", rt4801h_client->config.gpio_vsp_enable);
		return retval;
	}
	/* delay 10us */
	udelay(10);

	retval = gpio_direction_input(rt4801h_client->config.gpio_vsn_enable);
	if (retval != 0) {
	...
	retval = gpio_direction_input(rt4801h_client->config.gpio_vsp_enable);
	if (retval != 0) {
	...
	gpio_free(rt4801h_client->config.gpio_vsn_enable);
	gpio_free(rt4801h_client->config.gpio_vsp_enable);
