...
static void init_freqs_data(const struct cpufreq_policy *policy)
{
	...
	mem_size = (int)(len * (sizeof(unsigned int) + sizeof(u64) + sizeof(u64)));
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	fg_freqs_threshold = (unsigned int)value;
	return count;
}
	...
	int threhold_up, int threhold_down)
{
	...
	if (total_delta_time != 0) {
		...
		if (busy_precent >= (threhold_up * (int)total_delta_time)) {
			high_load_cnt[type]++;
			cycle_big_high_cnt[type]++;
		}

		if (busy_precent < (threhold_down * (int)total_delta_time)) {
...
static void high_freqs_load_tick(void)
{
	...
	now = ktime_get();
	delta_time = (u64)ktime_us_delta(now, last);
	last = now;
