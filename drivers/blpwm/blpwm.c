...
static void blpwm_store_gesture(uintptr_t arg)
{
	if (!arg) {
		hwlog_err("%s: input arg is null\n", __func__);
		return;
	...
	if (copy_from_user(&g_blpwm_dev->gesture, (void __user *)arg, sizeof(int))) {
		hwlog_err("%s: copy from user fail\n", __func__);
		return;
	}

	hwlog_info("%s: g_blpwm_dev->gesture is %d\n", __func__,
		g_blpwm_dev->gesture);
}
...
static void blpwm_set_gesture(void)
{
	if (g_blpwm_dev->gesture)
		blpwm_glink_config_adsp(g_blpwm_dev, GESTURE_OPEN);
	else
		blpwm_glink_config_adsp(g_blpwm_dev, GESTURE_CLOSE);
}
...
static long blpwm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_COVER_LOOKUP_CMD:
		...
		break;
	case IOCTL_BLPWM_GESTURE:
		blpwm_store_gesture((uintptr_t) data);
		blpwm_set_gesture();
		break;
	default:
