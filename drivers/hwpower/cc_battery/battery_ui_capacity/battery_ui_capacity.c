...
#include <huawei_platform/antenna_cable/antenna_cable_state.h>
...
int bat_ui_capacity(void)
{
	...
	if (di->force_zero && (di->ui_capacity <= BUC_CAPACITY_LOW)) {
		hwlog_info("ui_cap forced to 0\n");
		return BUC_CAPACITY_EMPTY;
	}
...
int bat_ui_raw_capacity(void)
{
	...
	if (di->force_zero && (cap <= BUC_CAPACITY_LOW)) {
		hwlog_info("ui_cap forced to 0\n");
		return BUC_CAPACITY_EMPTY;
	}
...
static void bat_ui_cable_detect_work(struct work_struct *work)
{
	struct bat_ui_capacity_device *di = container_of(work,
		struct bat_ui_capacity_device, detect_work.work);

	if (!cable_out_get()) {
		di->force_zero = true;
		hwlog_info("bat_ui_cable_detect_work\n");
	}
}

static int bat_ui_capacity_ui_event_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	...
	switch (event) {
	case POWER_NE_UI_LOW_SOC_FORCE_ZERO:
		if (bat_model_get_bat_cathode_type() !=BAT_MODEL_BAT_CATHODE_TYPE_SILICON)
			return NOTIFY_OK;
		if (!delayed_work_pending(&di->detect_work)) {
			queue_delayed_work(system_power_efficient_wq, &di->detect_work,
				msecs_to_jiffies(BUC_WORK_DETECT_CABLE_TIME));
			hwlog_info("delayed_work_pending over\n");
		}
		break;
	case POWER_NE_UI_LOW_POWER_FORCE_ZERO:
			di->force_zero = true;
		break;
	default:
		break;
...
static void bat_ui_wait_ready_work(struct work_struct *work)
{
	...
	if (coul_interface_get_battery_voltage(bat_core_get_coul_type()) < 0) {
		di->wait_cnt++;
		if (di->wait_cnt < BUC_WAIT_CNT_MAX) {
			hwlog_info("waiting battery gauge ic ready\n");
			queue_delayed_work(system_power_efficient_wq,
				&di->wait_work,
				msecs_to_jiffies(BUC_WAIT_INTERVAL));
		} else {
...
static void bat_ui_shutdown_correct_dts(struct bat_ui_capacity_device *di)
{
	...
	if (!di->correct_shutdown_soc_en)
		return;
	/* correct soc from shutdown_cap(1%) + shutdown_gap(2%) to shutdown_gap(1%) when voltage is below 3380 mV */
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
...
static void bat_ui_capacity_dts_init(struct bat_ui_capacity_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"force_zero_en", &di->force_zero_en, 0); /* it is an optional logic for silicon battery */
...
static int bat_ui_capacity_probe(struct platform_device *pdev)
{
	...
	di->ui_event_nb.notifier_call = bat_ui_capacity_ui_event_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_COUL, &di->ui_event_nb);
	if (ret)
	...
	INIT_DELAYED_WORK(&di->detect_work, bat_ui_cable_detect_work);
...
static int bat_ui_capacity_remove(struct platform_device *pdev)
{
	...
	power_event_bnc_unregister(POWER_BNT_CHARGING, &di->event_nb);
	power_event_bnc_unregister(POWER_BNT_COUL, &di->ui_event_nb);
