...
#define WAIT_SN_RETRY_MAX_TIMES          2
...
struct sensor_para {
	...
	char comp_para_index_fix[COMP_PARA_NODE_LEN_MAX];
	struct compensation_para comp_data[COMP_PARA_LEVEL];
	struct compensation_para comp_data_fix[COMP_PARA_LEVEL];
	int comp_para_size;
	int comp_para_size_fix;
	int temp;
	int raw;
};
...
struct multi_btb_temp_info {
	...
	int comp_para_id;
	bool batt_retry_match_done;
	int batt_types_match_times;
	int need_different_comp;
};
...
/* retry max two times (about 10s) if not match battery type, to avoid that battery sn has not yet get */
static void multi_btb_retry_match_another_battery_type(struct multi_btb_temp_info *di)
{
	if (!di->need_different_comp || di->batt_retry_match_done)
		return;

	di->batt_types_match_times++;
	if (bat_model_match_cob_battery()) {
		di->comp_para_id = SENSOR_INFO_COMP_PARA_ID_FIX;
		di->batt_retry_match_done = true;
		hwlog_info("another battery type is matched, use fix comp para\n");
		return;
	}

	hwlog_info("bat type match nothing, batt types match times is %d\n", di->batt_types_match_times);
	if (di->batt_types_match_times >= WAIT_SN_RETRY_MAX_TIMES)
		di->batt_retry_match_done = true;
}
...
static int multi_btb_get_temp_with_comp(int index, int temp_without_comp)
{
	...
	if (di->comp_para_id == SENSOR_INFO_COMP_PARA_ID_FIX) {
		data.para = di->sensor_tab[index].comp_data_fix;
		data.para_size = di->sensor_tab[index].comp_para_size_fix;
	} else {
...
static void multi_btb_temp_work(struct work_struct *work)
{
	...
	multi_btb_retry_match_another_battery_type(di);
	...
	struct multi_btb_temp_info *di)
{
	...
	const char *str = NULL;
	int sensor_info_total;

	/* if 3 columns of sensor-names is configed, will try to match different battery type and use different comp para */
	/* else will use default comp para */
	if (di->need_different_comp)
		sensor_info_total = SENSOR_INFO_TOTAL;
	else
		sensor_info_total = 2; /* 2: default sensor info total without different comp */
	...
	for (i = 0; i < array_len; i++) {
		...
		case SENSOR_INFO_COMP_PARA_ID:
			...
			break;
		case SENSOR_INFO_COMP_PARA_ID_FIX:
			strncpy(di->sensor_tab[row].comp_para_index_fix,
				str, COMP_PARA_NODE_LEN_MAX - 1);
			break;
		default:
			break;
...
static void multi_btb_temp_parse_comp_para_fix(struct device_node *np,
	struct multi_btb_temp_info *di, const int group)
{
	int array_len, col, row;
	int idata[COMP_PARA_LEVEL * COMP_PARA_TOTAL] = { 0 };
	...
	di->sensor_tab[group].comp_para_size_fix = 0;
	str = di->sensor_tab[group].comp_para_index_fix;
	array_len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		str, idata, COMP_PARA_LEVEL, COMP_PARA_TOTAL);
	if (array_len < 0) {
		di->need_different_comp = 0;
		return;
	}

	di->sensor_tab[group].comp_para_size_fix = array_len / COMP_PARA_TOTAL;
	for (row = 0; row < array_len / COMP_PARA_TOTAL; row++) {
		col = row * COMP_PARA_TOTAL + COMP_PARA_ICHG;
		di->sensor_tab[group].comp_data_fix[row].refer = idata[col];
		col = row * COMP_PARA_TOTAL + COMP_PARA_TEMP;
		di->sensor_tab[group].comp_data_fix[row].comp_value = idata[col];
		hwlog_info("temp_comp_para[%d]=%d %d\n", row,
			di->sensor_tab[group].comp_data_fix[row].refer,
			di->sensor_tab[group].comp_data_fix[row].comp_value);
	}
}
	...
	struct device_node *np, struct multi_btb_temp_info *di)
{
	...
	for (i = 0; i < di->sensor_num; i++) {
		...
		if (di->need_different_comp)
			multi_btb_temp_parse_comp_para_fix(np, di, i);
	}
}
	...
	struct multi_btb_temp_info *di)
{
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"need_different_comp", &di->need_different_comp, 0);
...
static int multi_btb_temp_probe(struct platform_device *pdev)
{
	...
	di->comp_para_id = SENSOR_INFO_COMP_PARA_ID;
