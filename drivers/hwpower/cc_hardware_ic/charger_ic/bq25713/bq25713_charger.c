...
static int bq25713_chip_init(struct charge_init_data *init_crit)
{
	int ret = -1;
...
static int bq25713_set_watchdog_timer(int value)
{
	...
	if (value >= CHAGRE_WDT_160S)
		reg_wdt = BQ25713_WATCHDOG_TIMER_LONG;
	else if (value >= CHAGRE_WDT_40S)
...
{
	return bq25713_set_watchdog_timer(timer_ms / POWER_MS_PER_S);
}
...
static int bq25713_batinfo_get_ibat(int *ibat_ma, void *data)
{
	...
	*ibat_ma = -power_platform_get_battery_current();
	return 0;
}
...
static void bq25713_scp_ops_register(void)
{
	...
	ret[0] = dc_ic_ops_register(LVC_MODE, IC_ONE, &bq25713_dc_ops);
	ret[1] = dc_ic_ops_register(SC_MODE, IC_ONE, &bq25713_dc_ops);
	ret[2] = dc_batinfo_ops_register(IC_ONE, &bq25713_batinfo_ops, DC_DEVICE_ID_END);
...
static int bq25713_ops_register(void)
{
	...
	if (charge_ops_register(&bq25713_ops, BUCK_IC_TYPE_THIRDPARTY))
		return -1;
	...
	const struct i2c_device_id *id)
{
	...
	di->wlock = power_wakeup_source_register(di->dev, "bq25713_wakelock");
