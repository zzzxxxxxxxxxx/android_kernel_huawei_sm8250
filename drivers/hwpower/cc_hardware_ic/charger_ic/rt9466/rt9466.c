...
static int rt9466_chip_init(struct charge_init_data *init_crit)
{
	int ret;
...
static struct hwfcp_ops rt9466_fcp_protocol_ops = {
	.chip_name = "rt9466",
	...
	const struct i2c_device_id *dev_id)
{
	...
	ret = charge_ops_register(&rt9466_ops, BUCK_IC_TYPE_THIRDPARTY);
	if (ret < 0) {
	...
	if (!rt9466_fcp_init(info))
		hwfcp_ops_register(&rt9466_fcp_protocol_ops);
