...
static int hl7139_config_tdie_otp_regulation(int tdie_regulation, void *dev_data)
{
	u8 value;
	int ret;
	struct hl7139_device_info *di = dev_data;

	if (tdie_regulation > HL7139_REG_CTRL0_TDIE_REG_110C)
		tdie_regulation = HL7139_REG_CTRL0_TDIE_REG_110C;
	if (tdie_regulation < HL7139_REG_CTRL0_TDIE_REG_80C)
		tdie_regulation = HL7139_REG_CTRL0_TDIE_REG_80C;

	value = (tdie_regulation - HL7139_REG_CTRL0_TDIE_REG_80C) /
		HL7139_REG_CTRL0_TDIE_REG_STEP;
	ret = hl7139_write_mask(di, HL7139_REG_CTRL0_REG, HL7139_REG_CTRL0_TDIE_REG_TH_MASK,
		HL7139_REG_CTRL0_TDIE_REG_TH_SHIFT, value);
	if (ret)
		return -EPERM;

	hwlog_info("config_tdie_otp_regulation [%x]=0x%x\n", HL7139_REG_CTRL0_REG, value);
	return 0;
}
...
static void hl7139_parse_dts(struct device_node *np, struct hl7139_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tdie_regulation", &di->tdie_regulation, HL7139_REG_CTRL0_TDIE_REG_100C);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"mount_on_fake_i2c", &di->mount_on_fake_i2c, HL7139_MOUNT_FAKE_I2C_DEFAULT_VAL);
}
