...
static char als_sensor_name[APP_INFO_VALUE_LENTH] = {0};
...
extern ssize_t show_send_lcdfreq_req(struct device *dev,
    struct device_attribute *attr, char *buf);
extern ssize_t store_send_ps_forcecali_req(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t size);
extern ssize_t show_send_ps_forcecali_req(struct device *dev,
    struct device_attribute *attr, char *buf);
...
static ssize_t attr_ps_send_forcecali_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return store_send_ps_forcecali_req(dev, attr, buf, count);
}

static ssize_t attr_ps_send_forcecali_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return show_send_ps_forcecali_req(dev, attr, buf);
}

static ssize_t attr_ps_forcecali_support_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return show_ps_forcecali_support(dev, attr, buf);
}
...
static DEVICE_ATTR(send_ps_forcecali_req, 0660, attr_ps_send_forcecali_show, attr_ps_send_forcecali_store);
static DEVICE_ATTR(get_forcecali_support, 0660, attr_ps_forcecali_support_show, NULL);
...
static struct attribute *proximity_attributes[] = {
    ...
    &dev_attr_send_ps_forcecali_req.attr,
    &dev_attr_get_forcecali_support.attr,
    NULL,
};
    ...
    struct device_attribute *attr, char *buf)
{
	...
	int ret = app_info_proc_get("lcd type", als_lcd_name);
	if (ret) {
		hwlog_err("%s, read L-Sensor type failed : %d!\n", __func__, ret);
	} else {
...
static ssize_t attr_gyro_get_sensor_id_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	memcpy_s(gyro_sensor_id, count, buf, count);
	return count;
}
...
{
	int ret;
	ret = memcpy_s(enable_drop, count, buf, count);
	if (ret != 0) {
		hwlog_err("%s:memcpy_s copy failed.\n", __func__);
		return count;
	}
...
void get_enable_drop_status_from_dts(struct device_node *dn)
{
	if (of_property_read_string(dn,
		...
		hwlog_err("%s, can't find enable_drop node sensorhub\n", __func__);
	}
	pr_info("enable_drop: %s\n", enable_drop);
	hwlog_info("%s, enable_drop: %s\n", __func__, enable_drop);
}
