...
static UINT32 hal_cali_info_support;
static UINT32 distinguish_product;
...
static int as7341_hal_cali_info_available(void)
{
    hwlog_info("%s, hal_cali_info_support = %d\n", __func__, hal_cali_info_support);
    return hal_cali_info_support;
}

static int as7341_distinguish_product_available(void)
{
...
static void configure_functions(struct color_chip *chip)
{
	...
	chip->color_hal_cali_info_available = as7341_hal_cali_info_available;
	chip->color_distinguish_product_available = as7341_distinguish_product_available;
...
static void read_options_parameter(struct device *dev)
{
	...
	rc = of_property_read_u32(dev->of_node, "hal_cali_info_support",
		&hal_cali_info_support);
	if (rc < 0) {
		hwlog_warn("%s, get hal_cali_info_support failed\n", __func__);
		hal_cali_info_support = 0;
	}

	rc = of_property_read_u32(dev->of_node, "distinguish_product",
		&distinguish_product);
	if (rc < 0) {
		hwlog_warn("%s, get distinguish_product failed\n", __func__);
		distinguish_product = 0;
	}

	hwlog_info("%s flicker_support = %u, algo_support = %u, hal_cali_info_support = %d, distinguish_product = %d\n",
		__func__, flicker_support, algo_support, hal_cali_info_support, distinguish_product);
}
