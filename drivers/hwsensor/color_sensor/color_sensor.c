...
static ssize_t hal_cali_info_available_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct color_chip *chip = NULL;
	int hal_cali_info_support;
	...
	if (chip->color_hal_cali_info_available == NULL) {
		return snprintf(buf, PAGE_SIZE, "%d\n", 0);
	}

	hal_cali_info_support = chip->color_hal_cali_info_available();
	hwlog_info("%s, hal_cali_info_support = %d\n", __func__, hal_cali_info_support);

	return snprintf(buf, PAGE_SIZE, "%d\n", hal_cali_info_support);
}

static ssize_t distinguish_product_available_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct color_chip *chip = NULL;
	int distinguish_product;
	...
	if (chip->color_distinguish_product_available == NULL) {
		return snprintf(buf, PAGE_SIZE, "%d\n", 0);
	}

	distinguish_product = chip->color_distinguish_product_available();
...
DEVICE_ATTR(hal_cali_info_available, 0660, hal_cali_info_available_show, NULL);
DEVICE_ATTR(distinguish_product_available, 0660, distinguish_product_available_show, NULL);
...
static struct attribute *color_sensor_attributes[] = {
	...
	&dev_attr_hal_cali_info_available.attr,
	&dev_attr_distinguish_product_available.attr,
