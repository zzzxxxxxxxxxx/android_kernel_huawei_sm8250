...
static int hall_irq_num[8];
static bool is_screen_on = true;
static bool is_pen_irq_disable = false;
static unsigned int pen_irq_cnt;
static unsigned int pen_both_on_cnt;
...
static unsigned int workaround_is_support;
...
static void process_pen_irq(unsigned int value)
{
	...
	if (is_screen_on) {
		pen_irq_cnt = 0;
		pen_both_on_cnt = 0;
		AK8789_INFOMSG("skip all irq when screen on\n");
		return;
	}
	...
	if (pen_irq_cnt == 0) {
		...
		AK8789_INFOMSG("set pre_ts first time when screen off\n");
	}
	...
	if (ns >= limit_time) {
		pen_irq_cnt = 0;
		pen_both_on_cnt = 0;
		AK8789_INFOMSG("sub_ts is over 10min\n");
	}
...
static int hall_gpio_irq_setup(struct device *dev)
{
	...
	list_for_each_entry(gpio_ptr, &hw_hall_dev.head, list) {
		...
		AK8789_INFOMSG("hall_value = %d, irq = %d, irq_flags = 0x%x\n", gpio_ptr->hall_value, irq, irq_flags);
	}
...
static int hall_drm_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if (!blank_event || !blank_event->data) {
		AK8789_INFOMSG("l_dev or blank_event is null\n");
		return NOTIFY_DONE;
	}
	...
	if ((event == DRM_PANEL_EVENT_BLANK) && (*blank == DRM_PANEL_BLANK_UNBLANK)) {
		...
		spin_unlock_irqrestore(&irq_lock, irq_flag);
		AK8789_INFOMSG("screen on! is_pen_irq_disable:%d\n", is_pen_irq_disable);
	} else if ((event == DRM_PANEL_EVENT_BLANK) && (*blank == DRM_PANEL_BLANK_POWERDOWN)) {
		is_screen_on = false;
		pen_irq_cnt = 0;
		pen_both_on_cnt = 0;
		AK8789_INFOMSG("screen off!\n");
	}
...
static struct delayed_work notify_wireless_charger_work;
static void support_set_notify_flag_work(struct work_struct *work)
{
	int hall_status;
	...
	workaround_is_support = get_support_hall_workaround_value();
	if (workaround_is_support) {
		lcd_kit_drm_notifier_register(0, &hall_drm_notify);
		duration = get_limit_duration_value();
		limit_time = duration * MINUTES_TO_NS;
		limit_irq_cnt = get_limit_pen_irq_cnt_value();
		limit_both_on_cnt = get_limit_pen_both_on_cnt_value();
		AK8789_INFOMSG("limit_time:%dmin, limit_irq_cnt:%d, limit_both_on_cnt:%d\n",
			duration, limit_irq_cnt, limit_both_on_cnt);
	}

	flag_value = get_support_set_notify_flag_value();
	if (!flag_value) {
		return;
	}
	set_notify_wireless_charger_value(flag_value);
	hall_status = query_hall_event();
	ext_hall_notify_event(hw_hall_dev.ext_hall_type, hall_status);
	AK8789_INFOMSG("update support_set_notify_flag_work");
}
...
static int hall_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&notify_wireless_charger_work, support_set_notify_flag_work);
	//Make sure the HuaweiShareManager service is up before notifying wireless charging.
	schedule_delayed_work(&notify_wireless_charger_work, msecs_to_jiffies(15000));
