...
#define PS_SENSOR_NEAR            0
#define PS_SENSOR_FAR             5
...
int thp_prox_event_report(int report_value[], unsigned int len)
{
	...
	struct custom_sensor_data_t psensor_data;
	...
	pr_info("%s:near_far = %d", __func__, near_far);
	psensor_data.sensor_data_head.sensor_type = SENSOR_TYPE_PROXIMITY + SENSOR_TYPE_PRIVATE_BASE_TYPE;
	psensor_data.sensor_data_head.action = STD_SENSOR_DATA_REPORT;
	memset(&psensor_data.sensor_data[0], 0, 16 * sizeof(int));
	psensor_data.sensor_data[0] = near_far == PS_SENSOR_NEAR ? PS_SENSOR_NEAR : PS_SENSOR_FAR;
	return ap_sensor_route_write((char*)&psensor_data,
