	...
	for (i = 0; i < cnt; i++) {
		...
		if (cm->wait) {
			if (cm->waittype == WAIT_TYPE_US)
				udelay((unsigned int)cm->wait);
			else if (cm->waittype == WAIT_TYPE_MS)
				mdelay((unsigned int)cm->wait);
			else
				mdelay((unsigned int)cm->wait * 1000); // change s to ms
		}
		cm++;
	}
	return 0;
...
void lcd_kit_gpio_tx(uint32_t panel_id, uint32_t type, uint32_t op)
{
	...
	case LCD_KIT_MIPI_SWITCH:
		...
		break;
	case LCD_KIT_TE_SWITCH:
		g_lcd_kit_gpio = power_hdl->lcd_te_switch.buf[1];
		break;
	...
	if (gpio_cm == NULL) {
		LCD_KIT_ERR("The gpio_cm pointer is null\n");
		return;
	}
