...
#define PLUG_OUT_DELAY_AFTER_MICBIAS     90
#define PLUG_OUT_DELAY_AFTER_EN          5
...
struct ana_hs_core_data {
	...
	int pmic_num;
	bool plugout_timing_optimization;
	unsigned int delay_time_after_micbais_off;
	unsigned int delay_time_after_en_off;
	unsigned int plugin_status_stable_delay;
...
static void plugout_work(struct work_struct *work)
{
	...
	while (count--) {
		if (g_data->ana_hs_in == ANA_HS_PLUG_IN) {
			...
			mutex_unlock(&g_data->mutex);
			if (g_data->plugout_timing_optimization) {
				msleep(g_data->delay_time_after_micbais_off);
				ana_hs_sequence_update(g_data->plug_out_seq);
				pinctrl_mic_gnd_swap(false);
				msleep(g_data->delay_time_after_en_off);
...
static void ana_hs_core_plug_handle(int hs_state)
{
	...
	case ANA_HS_PLUG_IN:
		queue_delayed_work(g_data->plugin_delay_wq,
			&g_data->plugin_delay_work,
			msecs_to_jiffies(g_data->plugin_status_stable_delay));
		break;
...
static void parse_extra_sequence_data(struct device_node *node)
{
	int ret;
	unsigned int temp = 0;

	ret = of_property_read_u32(node, "plugin_status_stable_delay", &temp);
	if (ret < 0) {
		g_data->plugin_status_stable_delay = PLUG_IN_STATUS_STABLE_DELAY;
	} else {
		g_data->plugin_status_stable_delay = temp;
		hwlog_err("%s: plugin_status_stable_delay = %u\n",
			__func__, g_data->plugin_status_stable_delay);
	}

	g_data->plugout_timing_optimization =
		of_property_read_bool(node, "plugout_timing_optimization");
	hwlog_info("%s: plugout_timing_optimization is configed\n", __func__);

	ret = of_property_read_u32(node, "delay_time_after_micbais_off", &temp);
	if (ret < 0) {
		g_data->delay_time_after_micbais_off = PLUG_OUT_DELAY_AFTER_MICBIAS;
	} else {
		g_data->delay_time_after_micbais_off = temp;
		hwlog_info("%s: plugin_status_stable_delay = %u\n",
			__func__, g_data->plugin_status_stable_delay);
	}
	ret = of_property_read_u32(node, "delay_time_after_en_off", &temp);
	if (ret < 0) {
		g_data->delay_time_after_en_off = PLUG_OUT_DELAY_AFTER_EN;
	} else {
		g_data->delay_time_after_en_off = temp;
		hwlog_info("%s: delay_time_after_en_off = %u\n",
			__func__, g_data->delay_time_after_en_off);
	}
}
...
static void parse_sequence_data(struct device_node *node)
{
	...
	parse_extra_sequence_data(node);
}
