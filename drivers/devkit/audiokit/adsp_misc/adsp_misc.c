...
static int adsp_misc_check_cmd(unsigned int cmd)
{
...
static int adsp_misc_get_buf(const void __user *arg,
	const void *data_ptr, int data_len)
{
	int ret = read_data_from_dsp(data_ptr, data_len);
	if (ret) {
	...
	void __user *arg, int compat_mode)
{
	...
	case SET_PA_PARAM:
		...
		break;
	case IPI_GET_BUF:
		ret = adsp_misc_get_buf(arg, data_ptr, data_len);
		if (ret)
			hwlog_err("cmd = %d, ret = %d\n", cmd, ret);
		break;
