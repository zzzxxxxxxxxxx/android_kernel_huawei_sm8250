...
static void boxid_gpio_status_show(char *buf)
{
	struct box_info_st *info = device_boxid.box_info;
	int i, j;
...
static void boxid_box_map_show(char *buf)
{
	struct box_info_st *info = device_boxid.box_info;
	int i;
...
static void boxid_box_name_show(char *buf)
{
	struct box_info_st *info = device_boxid.box_info;
	int i;
	int vendor;
	...
	struct device_attribute *attr, char *buf)
{
	...
	boxid_gpio_status_show(buf);

	boxid_box_map_show(buf);

	boxid_box_name_show(buf);
...
static void boxid_parse_tristate_mode(struct platform_device *pdev)
{
	int ret;

	/* read tristate_mode */
	ret = of_property_read_u32(pdev->dev.of_node, "tristate_mode",
...
static int boxid_proble_by_mode(struct platform_device *pdev)
{
	int ret;
...
static int boxid_probe(struct platform_device *pdev)
{
	...
	ret = get_boxid_info(pdev->dev.of_node);
	if (ret) {
	...
	boxid_parse_tristate_mode(pdev);

	ret = get_boxid_map(pdev->dev.of_node);
	if (ret) {
		pr_err("%s: get boxID_map info err\n", __func__);
		return -ENOENT;
	}

	ret = boxid_proble_by_mode(pdev);
	if (ret)
		return ret;
