...
static int watchdog_state_check(void)
{
	int timer_state = 0;
	int ret = 1; /* default value 1: no need to check watchdog state */
	u32 check_watchdog_state = g_ts_kit_platform_data.esd_watchdog_state_check;

	if (!check_watchdog_state)
		return ret;
	timer_state = timer_pending(&g_ts_kit_platform_data.watchdog_timer);
	TS_LOG_INFO("timer_state is %d\n", timer_state);
	if (!timer_state)
		ret = 0;
...
int ts_power_off_control_mode(int irq_id, struct ts_cmd_node *in_cmd)
{
	...
	case TS_BEFORE_SUSPEND: /* do something before suspend */
		...
		if (g_ts_kit_platform_data.poweroff_disable_irq == 1)
...
int ts_get_chip_info(struct ts_cmd_node *in_cmd)
{
	...
	get_watchdog_state = watchdog_state_check();
	if ((!g_ts_kit_platform_data.chip_data->is_parade_solution) && get_watchdog_state)
		ts_stop_wd_timer(&g_ts_kit_platform_data);
