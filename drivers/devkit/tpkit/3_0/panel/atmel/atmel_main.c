...
static int atmel_init_chip(void)
{
	...
	data->ts_flash_wake_lock = wakeup_source_register(
		&data->atmel_dev->dev, ATMEL_VENDER_NAME);
#else
	data->ts_flash_wake_lock = wakeup_source_register(ATMEL_VENDER_NAME);
#endif
	if (!data->ts_flash_wake_lock) {
		TS_LOG_ERR("atmel wakeup source register failed\n");
		return -EINVAL;
	}
