...
#define CTS_CSV_SRAM_TEST_ENABLE         "sram_test_enable"
#define CTS_CSV_MUTUAL_DEVIATION_LIMIT_M   "mutual_deviation_limit_m"
#define CTS_CSV_MUTUAL_DEVIATION_LIMIT_N   "mutual_deviation_limit_n"
...
#define CTS_CSV_NOISE_TEST_FRAME_IGNORE  "noise_test_frame_ignore"
...
struct cts_test_param {
	...
	int max_limits_col[CTS_MAX_DATA_NOTES];
	int max_limits_row[CTS_MAX_DATA_NOTES];
...
struct cts_sram_test_priv_param {
    __u32 frames;
};
...
int cts_test_deviation(struct cts_device *cts_dev,
		       struct cts_test_param *param);
int cts_test_sram(struct cts_device *cts_dev,
	struct cts_test_param *param);
