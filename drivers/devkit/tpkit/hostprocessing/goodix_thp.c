...
#define EVENT_STYLUS_SINGLE_CLICK 4
	...
	u8 *data, unsigned int len)
{
	...
	if ((tdev == NULL) || (data == NULL) || (tx_frame_buff == NULL) ||
	...
	struct thp_udfp_data *udfp_data)
{
	...
	if (udfp_data->key_event == EVENT_STYLUS_SINGLE_CLICK)
...
{
	unsigned int stylus3_bt_status;
	...
	thp_log_info("%s: stylus3_bt_status = %u, stylus_adsorption_status = %u\n",
	...
	bool delay_enable)
{
	...
	thp_log_info("%s: check data is: %d\n", __func__, bt_connect_value);
	...
	bool delay_enable)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9966) {
		ret = touch_driver_ai_bt_handler(tdev, delay_enable);
		if (ret)
			thp_log_err("touch_driver_ai_bt_handler: failed\n");
		else
			thp_log_info("touch_driver_ai_bt_handler: success\n");
		return ret;
	}
	...
	u8 *data)
{
	...
	cmd_send.data[0] = data[0]; /* double click cmd */
	cmd_send.data[1] = data[1]; /* single click cmd */
	cmd_send.data[2] = data[2]; /* stylus click cmd */
...
static int touch_driver_suspend_gesture(struct thp_device *tdev)
{
	...
	if (goodix_stylus_status)
		data[2] = STYLUS_SINGLE_CLICK;
