...
static unsigned long m_second(unsigned long long nsec, unsigned long divs)
{
	if ((long long)nsec < 0)
		nsec = -nsec;
...
void log_boot(char *str)
{
	...
	tmp = ts;
	hw_boottime[boot_log_count].m_second = m_second(tmp, 1000000000);
	strncpy((char*)&hw_boottime[boot_log_count].event, str, BOOT_STR_SIZE);
	printk("boottime %d.%d : %s\n", hw_boottime[boot_log_count].second, hw_boottime[boot_log_count].m_second, hw_boottime[boot_log_count].event);
	boot_log_count++;
...
static int hw_boottime_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < boot_log_count; i++)
		seq_printf(m, "%d.%09u s : %s\n", hw_boottime[i].second, hw_boottime[i].m_second, hw_boottime[i].event);
