...
char *wifipro_ntoa(int addr)
{
	int len = 0;
	int res;
	static char buf[18] = {0}; /* 18 is the size of addr */

	res = memset_s(buf, sizeof(buf), 0, sizeof(buf));
	if (res != 0) return buf;
	/* 8 low addr 16 mid addr 24 high addr */
	res = sprintf_s(buf + len, ADDR_LENGTH, "%d.%d.%d.%d",
	...
	if ((1 << sk->sk_state) & TCPF_SYN_SENT) {
		if (skb->dev != NULL) {
			wifipro_dev_max_len = strnlen(skb->dev->name,
				IFNAMSIZ - 1);
			ret = strncpy_s(sk->wifipro_dev_name, wifipro_dev_max_len,
				skb->dev->name, wifipro_dev_max_len - 1);
			if (ret != 0) return;
...
int wifipro_handle_retrans(struct sock *sk, struct inet_connection_sock *icsk)
{
	...
	if (!wifipro_is_not_local_or_lan_sock(dest_addr) ||
		sk->wifipro_is_google_sock) return 0;
	...
	unsigned int quality)
{
	...
	res = memset_s(packet, sizeof(struct wifipro_nl_packet_msg),
		0, sizeof(struct wifipro_nl_packet_msg));
	if (res != 0) {
		kfree_skb(skb);
		skb = NULL;
		ret = -1;
	...
	unsigned char offset, const char *name)
{
	...
	(void)strncpy_s(dst->state_name, WIFIPRO_MAX_CA_NAME,
		name, WIFIPRO_MAX_CA_NAME - 1);
...
static void solve_uid_state_print(struct seq_file *seq)
{
	...
	res = memset_s(s_uid_wlan_tcp_stat, sizeof(s_uid_wlan_tcp_stat),
		0, sizeof(s_uid_wlan_tcp_stat));
	if (res != 0) return;
	s_wlan_tcp_stat_index = 0;
	...
	res = memset_s(s_uid_rmnet_tcp_stat, sizeof(s_uid_rmnet_tcp_stat),
		0, sizeof(s_uid_rmnet_tcp_stat));
	if (res != 0) return;
	s_rmnet_tcp_stat_index = 0;
}
...
static void update_hidata_tcp_stat_by_uid(kuid_t uid, int index, int count)
{
	...
	if (result == INVALID_INDEX) {
		...
		res = memset_s(&s_uid_hidata_tcp_stat[temp],
			sizeof(hidata_ui_dtcp_stat), 0, sizeof(hidata_ui_dtcp_stat));
		if (res != 0) return;
