...
static struct error_info_to_category error_info_categorys[] = {
	...
	{
		MODULE_SYSTEM,
		{EVENT_POWEROFF, CATEGORY_SYSTEM_POWEROFF, TOP_CATEGORY_SYSTEM_RESET}
	},
	{
		MODULE_SYSTEM,
...
static void format_log_dir(char *buf, size_t buf_size,
			   const char *log_root_dir, const char *timestamp);
			   ...
			   const char *log_root_dir, const char *timestamp)
{
	...
	(void)scnprintf(buf, buf_size - 1, "%s/%s", log_root_dir, timestamp);
}
			     ...
			     struct error_info *info, const char *timestamp, int need_sys_reset)
{
	...
	(void)full_write_file(history_log_path, buf, strlen(buf), 1);
...
static bool is_log_part_mounted(void)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	ret = ksys_access(CONFIG_BLACKBOX_LOG_PART_REPRESENTATIVE, 0);
	set_fs(old_fs);
...
static void wait_for_log_part(void)
{
	bbox_print_info("wait for log root part [%s] begin!\n",
			CONFIG_BLACKBOX_LOG_PART_REPRESENTATIVE);
	while (!is_log_part_mounted())
		msleep(LOG_PART_WAIT_TIME);

	bbox_print_info("wait for log root part [%s] end!\n",
			CONFIG_BLACKBOX_LOG_PART_REPRESENTATIVE);
}
...
static void do_save_last_log(const struct bbox_ops *ops, const struct error_info *info)
{
	...
	bbox_print_info("event: [%s] module: [%s] log, time is [%s]!\n", info->event, info->module, info->error_time);
	...
	if (ops->ops.save_last_log(log_dir, (struct error_info *)info) == 0)
		save_history_log(CONFIG_BLACKBOX_LOG_ROOT_PATH, (struct error_info *)info, info->error_time, 1);
	else
...
static void print_module_ops(void)
{
	...
	list_for_each_safe(cur, next, &ops_list) {
		...
		if (!temp)
			continue;
		bbox_print_info("module: %s, dump: %p, reset: %p,get_last_log_info: %p, save_last_log: %p\n",
				temp->ops.module, temp->ops.dump, temp->ops.reset,
				temp->ops.get_last_log_info, temp->ops.save_last_log);
	}
}
#endif
...
static void __exit blackbox_core_exit(void)
{
}

core_initcall(blackbox_core_init);
module_exit(blackbox_core_exit);
