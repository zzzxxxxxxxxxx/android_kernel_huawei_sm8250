...
static offset_info_struct offset_info;
	...
	uint64_t buf_size)
{
	...
	if (unlikely(dev_path == NULL) || unlikely(buf == NULL)) {
...
write_offset_to_raw_part(struct work_struct *work)
{
	...
	ret = write_emmc_raw_part(BSPINFO_RAW_PART_PATH, RTC_PART_OFFSET,
				(char *)&offset_info, sizeof(offset_info_struct));
	if (ret != 0)
...
int huawei_rtc_init(struct device *dev)
{
	...
	memcpy(&offset_info, smem->rtc_offset_info, sizeof(offset_info_struct));
	...
	if (strcmp(OFFSET_TAG, offset_info.off_tag)) {
		...
		memset(&offset_info, 0, sizeof(offset_info_struct));
