...
#define CLONEDPROFILE_USER_ID_MIN	128
#define CLONEDPROFILE_USER_ID_MAX	148
		...
		struct sdcardfs_inode_data *data)
{
	...
	else if (data->userid >= CLONEDPROFILE_USER_ID_MIN
			&& data->userid <= CLONEDPROFILE_USER_ID_MAX)
		return multiuser_get_uid(0, vfsopts->gid);
	else
