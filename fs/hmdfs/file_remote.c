...
static void hmdfs_set_loopbacknet_ra_pages(struct hmdfs_peer *peer,
					   struct file_ra_state *ra)
{
	if (!peer)
		return;
	if (is_loopback_device(peer))
		ra->ra_pages = 1024;
}
...
retry:
	...
	/* if net is loopback, ra_pages default 1024 */
	hmdfs_set_loopbacknet_ra_pages(info->conn, ra);
