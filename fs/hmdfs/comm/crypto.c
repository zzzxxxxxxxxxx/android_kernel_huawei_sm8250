		 ...
		 void *dst, size_t dst_len)
{
	...
	res = aeadcipher_encrypt_buffer(d2dp->send_tfm, src, src_len,
					dst, dst_len);
	if (res) {
		hmdfs_err("encrypt_buf fail: %d", res);
		return res;
	}
		 ...
		 void *dst, size_t dst_len)
{
	...
	res = aeadcipher_decrypt_buffer(d2dp->recv_tfm, src, src_len,
					dst, dst_len);
	if (res) {
		hmdfs_err("decrypt_buf fail: %d", res);
		return res;
	}
...
static int d2dp_update_tx_key(struct d2dp_handle *d2dp)
{
	...
	if (ret < 0) {
		hmdfs_err("D2DP TX key error on update: %d", ret);
		goto out_memzero;
	}
	...
	if (ret) {
		hmdfs_err("D2DP TX key error on lock: %d", ret);
		goto out_memzero;
	}
	...
	if (ret) {
		hmdfs_err("D2DP TX key error on crypto aead: %d", ret);
		goto out_memzero;
	}
...
static int d2dp_update_rx_key(struct d2dp_handle *d2dp)
{
	...
	if (ret < 0) {
		hmdfs_err("D2DP RX key error on update: %d", ret);
		goto out_memzero;
	}
	...
	if (ret) {
		hmdfs_err("D2DP RX key error on crypto aead: %d", ret);
		goto out_memzero;
	}
...
static int set_crypto_info_d2dp(struct d2dp_handle *d2dp, int set_type)
{
	...
	/*
	 * In the current HMDFS protocol design, both peers may initiate rekey
	 * process simultaneously. Moreover, both HMDFS recv thread and user
	 * application thread can start the process. It works for TLS case, but
	 * doesn't work for D2DP because of the following transport differences:
	 *
	 * 1. D2DP uses separate transport threads and actual {en,de}cryption of
	 *    data packets is done asynchronously with HMDFS threads.
	 * 2. ACK packets are sent independently from DATA packets.
	 * 3. D2DP encrypts all its headers, including the ACK header.
	 *
	 * From the one hand, if we switch our TX keys too early, then other
	 * peer may be blocked forever waiting for our ACK to proceed. From the
	 * other hand, if we switch our RX keys too early, we may not receive
	 * ACK from another peer to unblock our thread blocked on TX update
	 * path.
	 *
	 * Right now it is impossible to implement synchronized key update for
	 * D2DP transport without changing HMDFS protocol itself. To not break
	 * backward compatibility of HMDFS protocol, just insert some delay here
	 * before updating keys to let the peers send enough D2DP ACKs and thus
	 * prevent lockups. The lockups should not happen with such delay value,
	 * but even if it happens, the update proceess will be interrupted by
	 * D2DP RTO mechanism and the connection will be closed gracefully.
	 *
	 * 1 second waiting should be enough as wireless networks can induce up
	 * to 500ms delay and D2DP ACK rate is ~100 pps which leads to
	 * sufficient amount of ACKs during this delay. 1 second delay is also
	 * tolerable as rekey happens rare, and the rekey process may not happen
	 * at all if the user does not use a single HMDFS connection for all the
	 * rekey period.
	 */
	msleep(1000);

	if (set_type == SET_CRYPTO_SEND) {
		hmdfs_info("D2DP TX key update start");
		err = d2dp_update_tx_key(d2dp);
		if (!err)
			hmdfs_info("D2DP TX key updated successfully");
		else
			hmdfs_err("D2DP TX key update failed");
	}
	if (set_type == SET_CRYPTO_RECV) {
		hmdfs_info("D2DP RX key update start");
		err = d2dp_update_rx_key(d2dp);
		if (!err)
			hmdfs_info("D2DP RX key updated successfully");
		else
			hmdfs_err("D2DP RX key update failed");
	}
