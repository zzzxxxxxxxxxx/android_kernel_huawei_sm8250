					...
					bool is_recv_thread)
{
	...
	uid_t user_mnt_uid = multiuser_get_uid(sbi->user_id, sbi->mnt_uid);
	gid_t user_mnt_everybody_gid = multiuser_get_uid(sbi->user_id, AID_EVERYBODY);

	cred = prepare_creds();
	if (!cred)
		return NULL;

	cred->fsuid = KUIDT_INIT(user_mnt_uid);
	cred->fsgid = is_recv_thread ? KGIDT_INIT(user_mnt_everybody_gid) :
				       KGIDT_INIT(user_mnt_uid);
					    ...
					    struct dentry *dentry, __u16 *_perm)
{
	...
	case HMDFS_PERM_MNT:
		/* system : media_rw */
		cred->fsuid = KUIDT_INIT(multiuser_get_uid(sbi->user_id,
					 AID_SYSTEM));
	...
	cred->fsgid = KGIDT_INIT(multiuser_get_uid(sbi->user_id, AID_MEDIA_RW));
	*_perm = perm;
	old_cred = override_creds(cred);
		     ...
		     uid_t uid)
{
	...
	struct hmdfs_sb_info *sbi =  hmdfs_sb(child->i_sb);
	...
	if (!err)
		child->i_uid = KUIDT_INIT(user_uid);
	else
		     ...
		     uid_t uid, gid_t gid)
{
	...
	if (!err) {
		child->i_uid = KUIDT_INIT(user_uid);
		child->i_gid = KGIDT_INIT(user_gid);
	} else {
		hmdfs_err("update PKG uid failed, err = %d", err);
	}
}
 ...
 * other : media_rw : media_rw
 **/
static void init_remote_dir_uid_perm(struct dentry *dentry, __u16 *perm,
				     __u16 level)
{
	struct hmdfs_sb_info *sbi =  hmdfs_sb(dentry->d_sb);
	uid_t user_system_uid = multiuser_get_uid(sbi->user_id, AID_SYSTEM);
	...
	if (!strcmp(dentry->d_name.name, PKG_ROOT_NAME)) {
		/* data */
		d_inode(dentry)->i_uid = KUIDT_INIT(user_system_uid);
		*perm = HMDFS_DIR_DATA | level;
	} else if (!strcmp(dentry->d_name.name, SYSTEM_NAME)) {
		 /* system */
		d_inode(dentry)->i_uid = KUIDT_INIT(user_system_uid);
		*perm = AUTH_SYSTEM | HMDFS_DIR_SYSTEM | level;
		...
		d_inode(dentry)->i_uid = KUIDT_INIT(user_media_rw_uid);
		*perm = HMDFS_DIR_PUBLIC | level;
	}
}
				      ...
				      struct dentry *dentry)
{
	...
	case HMDFS_PERM_MNT:
		/* system : media_rw */
		dinode->i_uid = KUIDT_INIT(multiuser_get_uid(sbi->user_id, AID_SYSTEM));
		perm = (hii->perm & HMDFS_DIR_TYPE_MASK) | level;
		break;
	case HMDFS_PERM_DFS:
		init_remote_dir_uid_perm(dentry, &perm, level);
		break;
	case HMDFS_PERM_PKG:
		if (is_data_dir(hii->perm)) {
			/*
	...
	dinode->i_gid = KGIDT_INIT(multiuser_get_uid(sbi->user_id, AID_MEDIA_RW));
	dinfo->perm = perm;
}
...
void hmdfs_root_inode_perm_init(struct inode *root_inode)
{
	...
	struct hmdfs_sb_info *sbi =  hmdfs_sb(root_inode->i_sb);
	uid_t user_system_uid = multiuser_get_uid(sbi->user_id, AID_SYSTEM);
	gid_t user_media_rw_gid = multiuser_get_uid(sbi->user_id, AID_MEDIA_RW);

	hii->perm = HMDFS_DIR_ROOT | HMDFS_PERM_MNT;
	set_inode_uid(root_inode, KUIDT_INIT(user_system_uid));
	set_inode_gid(root_inode, KGIDT_INIT(user_media_rw_gid));
}
