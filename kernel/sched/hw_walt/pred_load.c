...
unsigned long predict_util(struct rq *rq)
{
	...
	util = div64_u64(rq->sum_pred_load << SCHED_CAPACITY_SHIFT, predl_window_size);
	util = max(rq->wrq.walt_stats.pred_demands_sum_scaled, (u64)util);
#else
...
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0) )
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE
	return max((u64)p->wts.pred_demand_scaled,
		div64_u64(p->ravg.predl << SCHED_CAPACITY_SHIFT, predl_window_size));
#else
...
#endif
#else
	return div64_u64(p->ravg.predl << SCHED_CAPACITY_SHIFT, predl_window_size);
#endif
}
		...
		struct task_struct *p, u32 runtime)
{
	...
	if (get_walt_mark_start(p) + predl_window_size < rq->predl_window_start)
		return runtime;
		...
		struct task_struct *p, bool rollover)
{
	...
	if (rollover)
		hw_bucket_increase(p->ravg.predl_busy_buckets, bidx);
#else
...
void predl_window_size_change_handle(u64 wc)
{
	...
	if ((predl_window_size != new_predl_window_size) &&
	    (wc < this_rq()->predl_window_start + new_predl_window_size)) {
