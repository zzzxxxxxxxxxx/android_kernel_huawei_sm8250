...
#ifdef CONFIG_HW_CPUFREQ_UTIL_FREQ
#include "../kernel/sched/hw_schedutil/cpufreq_util_freq_hw.h"
#endif
...
unsigned int sched_get_group_id(struct task_struct *p)
{
	...
	group_id = grp ? (unsigned int) grp->id : 0;
	rcu_read_unlock();
...
#ifdef CONFIG_HW_CGROUP_RTG
void add_new_task_to_grp_with_compatibility(struct task_struct *new)
{
	struct related_thread_group *grp = NULL;
	unsigned long flag;
...
int set_task_rtg_min_freq(struct task_struct *p, unsigned int freq)
{
	...
	unsigned long flags;
	int ret = 0;
#if defined(CONFIG_UCLAMP_TASK) && !defined(CONFIG_SCHED_TASK_UTIL_CLAMP)
	...
	util = hw_freq_to_util(cluster_cpu, freq);
#else
