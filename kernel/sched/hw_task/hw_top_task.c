...
{
	memset_s(table, NUM_LOAD_INDICES * sizeof(struct top_task_entry),
		 0, NUM_LOAD_INDICES * sizeof(struct top_task_entry));
}

static inline void clear_top_tasks_bitmap(unsigned long *bitmap)
{
	memset_s(bitmap, BITS_TO_LONGS(NUM_LOAD_INDICES) * sizeof(unsigned long),
		 0, BITS_TO_LONGS(NUM_LOAD_INDICES) * sizeof(unsigned long));
}
