...
#if defined(CONFIG_HW_VIP_SEMAPHORE) && defined(CONFIG_HW_VIP_THREAD)
		sem->m_count &= (RWSEM_FLAG_VIP | RWSEM_OWNER_BOOST);
#else
		sem->m_count = 0;
#endif
		return true;
...
#if defined(CONFIG_HW_VIP_SEMAPHORE) && defined(CONFIG_HW_VIP_THREAD)
		&& ((sem->m_count & ~(RWSEM_FLAG_VIP | RWSEM_OWNER_BOOST)) < RWSEM_MAX_PREEMPT_ALLOWED)) {
#else
