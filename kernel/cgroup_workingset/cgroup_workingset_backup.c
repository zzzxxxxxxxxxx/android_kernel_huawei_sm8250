...
static unsigned int g_max_records_count __read_mostly = MAX_RECORD_COUNT_ON_1G;
...
void workingset_writeback_all_records(void)
{
	...
	g_record_cnt += (unsigned int)total_records;
	spin_unlock(&g_record_list_lock);
	...
	struct file *filp, struct s_ws_backup_record_header *header)
{
	...
	if (header->playload_length >
	    (MAX_TOUCHED_PAGESEQ_COUNT * sizeof(unsigned int)) +
