	...
	const __be16 dc_proto, const u16 sequence)
{
	...
	/* data: dstMac[6] | srcMac[6] | protoType[2] */
	ret = memmove_s(skb->data, ETH_ALEN * MAC_ADDR_NUM, skb->data + DC_HLEN,
...
static void dc_header_rollback(struct sk_buff *skb)
{
	...
	/* data: dstMac[6] | srcMac[6] | protoType[2] */
	ret = memmove_s(skb->data + DC_HLEN, ETH_ALEN * MAC_ADDR_NUM,
		skb->data, ETH_ALEN * MAC_ADDR_NUM);
	if (ret != 0) return;
...
static void delete_dc_header(struct sk_buff *skb, u16 *sequence)
{
	...
	*sequence = ntohs(dchdr->sequence);
	...
	/* data: dstMac[6] | srcMac[6] | protoType[2] */
	ret = memmove_s(skb->data - ETH_HLEN, ETH_ALEN * MAC_ADDR_NUM,
		skb->data - DC_ETH_HLEN, ETH_ALEN * MAC_ADDR_NUM);
	if (ret != 0) return;
	skb->mac_header += DC_HLEN;
	...
	const struct net_device *dev)
{
	...
	ret = memcpy_s(skb->data + ETH_ALEN, ETH_ALEN,
		dev->dev_addr, ETH_ALEN);
	if (ret != 0)
		dc_mloge("memcpy_s fail");
}
