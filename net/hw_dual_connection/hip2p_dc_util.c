	...
	const struct net_device *slave_dev)
{
	...
	ret = memset_s(dc, sizeof(*dc), 0, sizeof(*dc));
	if (ret != 0) {
		dc_mlogd("dc create memset_s fail");
		return NULL;
	}
	dc_mlogd("set dc");
...
static void dc_rcu_free(struct rcu_head *head)
{
	...
	ret = memset_s(dc, sizeof(*dc), 0, sizeof(*dc));
	if (ret != 0)
		dc_mloge("dc rcu free memset_s fail");
}
...
static void clear_bitmap(struct bitmap *bitmap)
{
	...
	ret = memset_s(bitmap->map, sizeof(bitmap->map),
		0, sizeof(bitmap->map));
	if (ret != 0) {
		dc_mloge("lclear bitmap memset_s fail");
		return;
	}
	bitmap->threshold = 0;
}
...
int dcm_init(void)
{
	...
	ret = memset_s(&g_dcm, sizeof(struct dc_mngr),
		0, sizeof(struct dc_mngr));
	spin_lock_init(&g_dcm.dc_lock);
	if (ret != 0) {
		dc_mlogd("dc init memset_s fail");
		return HIP2P_FAIL;
	}
	return HIP2P_SUCC;
}
...
void dcm_exit(void)
{
	...
	ret = memset_s(&g_dcm, sizeof(struct dc_mngr),
		0, sizeof(struct dc_mngr));
	if (ret != 0)
		dc_mlogd("dc exit memset_s fail");
}
