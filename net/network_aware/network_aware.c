...
void stat_bg_network_flow_x(bool is_recving, int len)
{
	...
	if (period == 0) {
		...
		for (i = 1; i < period; i++) {
			...
			ret = memset_s(temp_net_stat_x, sizeof(*temp_net_stat_x), 0,
	...
	size_t count, loff_t *ppos)
{
	...
	int ret = memset_s(buffer, sizeof(buffer), 0, sizeof(buffer));
	if (ret != 0) {
		err = -EFAULT;
		goto out;
	}
	...
	size_t count, loff_t *ppos)
{
	...
	netinfo.bg_num = i;
	ret = memset_s(&netinfo.bg_net_stat, sizeof(netinfo.bg_net_stat),
		0, sizeof(netinfo.bg_net_stat));
	if (ret != 0) {
		err = -EFAULT;
		goto out;
	}
	...
	size_t count, loff_t *ppos)
{
	...
	int ret = memset_s(buffer, MAX_ARRAY_LENGTH, 0, sizeof(buffer));
	if (ret != 0) {
		err = -EFAULT;
		goto out;
	}
