...
static char *mas_blk_io_type(u64 op, unsigned int flag, u64 mas_flag, char *type_buf, int len)
{
	...
	offset += mas_blk_io_type_flag_unistore(
		mas_flag, type_buf + offset, len - offset);
#endif
	...
	struct blk_dev_lld *lld, char *buf, int len)
{
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "\tFGIO/Busy/Idle Notifier: %s\t",
		(lld->features & BLK_LLD_BUSYIDLE_SUPPORT) ? "enable"
							     : "disable");
	offset += __mas_blk_dump_printf(buf, len, tmp_str);
	...
	case BLK_FG_IO_BUSY:
		snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "\tFG State: BUSY\t");
		break;
	case BLK_FG_IO_IDLE:
		snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "\tFG State: IDLE\t");
		break;
	default:
		return offset;
	}
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "\tFG IO Count: %d\n",
	...
	struct blk_dev_lld *lld, char *buf, int len, char *tmp_str)
{
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "Write Order: %s Order Num: %u"
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "expected_lba: "
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "expected_lba_time: "
		"%llu, %llu, %llu, %llu\n",
	...
	snprintf(tmp_str, MAS_BLK_DUMP_BUF_LEN, "old_section: "
		...
		if (list_empty_careful(&lld->buf_bio_list[stream]))
			...
			list_for_each_entry(pos, &lld->buf_bio_list[stream], buf_bio_list_node)
				pr_err("recovery_dump, stream: %u, bio_nr: %u, "
					...
					(pos->bi_iter.bi_sector >> SECTION_SECTOR) %
						(lld->mas_sec_size),
					pos->bi_iter.bi_bvec_done / BLKSIZE);
