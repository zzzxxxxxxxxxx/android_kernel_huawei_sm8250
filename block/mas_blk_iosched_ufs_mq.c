...
#ifdef CONFIG_MAS_UNISTORE_PRESERVE
bool mas_blk_is_disorder_stuck(struct bio *bio)
{
	sector_t start_lba;
	unsigned long flags;
	...
	struct blk_dev_lld *lld = mas_blk_get_lld(bio->bi_disk->queue);
	if (!lld) {
	...
	if (likely(p))
		start_lba = (bio->bi_iter.bi_sector + p->start_sect) >> SECTION_SECTOR;
	else
	...
	if (atomic_read(&sched_ds_lld->mq_tag_used_cnt) <
		(MAS_MQ_QUEUE_DEPTH - 2 * UNISTORE_MQ_QUEUE_DEPTH - 8))
		return false;
	...
	const struct blk_mq_alloc_data *data)
{
	...
	if (data->mas_cmd_flags & MAS_REQ_LBA) {
...
bool mas_blk_is_reserved_empty(struct blk_mq_alloc_data *data)
{
	return (blk_queue_query_unistore_enable(data->q) &&
		!(data->mas_cmd_flags & MAS_REQ_RECOVERY) &&
		!(data->mas_cmd_flags & MAS_REQ_RESET) &&
	...
	const struct blk_mq_alloc_data *data)
{
	...
	if (data->mas_cmd_flags & MAS_REQ_RECOVERY)
		return ufs_mq_get_recovery_tag(data);

	if (data->mas_cmd_flags & MAS_REQ_RESET)
		return ufs_mq_get_all_tag(data);

	if ((flag & REQ_FG_META) || (data->mas_cmd_flags & MAS_REQ_LBA))
		tag = ufs_tagset_get_high_prio_tag(data);
	...
	struct bio *bio, struct mas_ufs_sched_ds_lld *ds_lld)
{
	...
	struct request *rq = NULL;
	struct blk_mq_hw_ctx *hctx = NULL;
	struct blk_mq_alloc_data alloc_data = {.mas_cmd_flags = MAS_REQ_RECOVERY};
	...
	ufs_mq_bio_to_request(rq, bio, nr_segs);
	hctx = blk_mq_map_queue(rq->q, rq->cmd_flags, rq->mq_ctx);
	bd.rq = rq;
	ret = __ufs_mq_queue_rq_internal(hctx, q, &bd, ds_lld);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
	blk_mq_put_ctx(alloc_data.ctx);
