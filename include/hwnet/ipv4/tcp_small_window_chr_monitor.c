...
int tcp_small_win_init_proc(struct net *net)
{
	if (!proc_create("chr_small_window",
		...
		net->proc_net, &g_tcp_small_window_seq_fops)) return -ENOMEM;
	if (!proc_create("chr_small_win_cnt",
		(S_IRUSR | S_IRGRP) | (S_IWUSR | S_IWGRP),
		net->proc_net, &g_tcp_small_win_cnt_seq_fops)) return -ENOMEM;
	if (!proc_create("chr_min_report_time",
		(S_IRUSR | S_IRGRP) | (S_IWUSR | S_IWGRP),
		net->proc_net, &g_tcp_min_report_time_seq_fops)) return -ENOMEM;
	if (!proc_create("chr_max_report_cnt",
		(S_IRUSR | S_IRGRP) | (S_IWUSR | S_IWGRP),
		net->proc_net, &g_tcp_max_report_cnt_seq_fops)) return -ENOMEM;
