...
static void proc_init_file_content(int file_index, int file_len)
{
	...
	ret = memset_s(g_configs.buffer[file_index], file_len, 0, file_len);
	if (ret != 0) return;
...
static void proc_update_packet_pcap_header(char *file_buffer, int packet_len, int orig_len)
{
	...
	ret = memcpy_s(file_buffer, sizeof(oam_pcap_pkthdr_stru), (char *)&st_pcap_pkthdr,
		sizeof(oam_pcap_pkthdr_stru) -1);
	if (ret != 0)
		printk(KERN_ERR "proc_update_packet_pcap_header, memcpy_s  fail");
}
	...
	int header_len, const char *frame_buff, int frame_len, int flag_rx_tx, int orig_len)
{
	...
	if (header_buff != NULL) {
		ret = memcpy_s(dst_buff + sizeof(oam_pcap_pkthdr_stru), header_len,
			header_buff, header_len -1);
		if (ret != 0) {
			printk(KERN_ERR "proc_write_packet_to_file, memcpy_s  fail");
			return;
		}
	}

	int ret = memcpy_s(dst_buff + sizeof(oam_pcap_pkthdr_stru) + header_len,
		frame_len, frame_buff, frame_len);
	if (ret != 0)
		printk(KERN_ERR "proc_write_packet_to_file, memcpy_s  fail");
}
	...
	const char channel_type)
{
	...
	if (channel < 36) {
		...
		ret = snprintf_s(new_cmds, sizeof(new_cmds), sizeof(new_cmds) - 1,
			cmd_str, get_mode_string(expand_type, g_wifi_chip_type));
		if (ret < 0) return;
	} else {
		...
		if (channel == 165) { // ch#165 only support VHT20
			if (g_wifi_chip_type == CHIPTYPE_1105) {
				ret = snprintf_s(new_cmds, sizeof(new_cmds),
					sizeof(new_cmds) - 1, cmd_str, "11ax5g20");
				if (ret < 0) return;
			} else {
				ret = snprintf_s(new_cmds, sizeof(new_cmds),
					sizeof(new_cmds) - 1, cmd_str, "11ac20");
				if (ret < 0) return;
			}
		} else {
			if ((channel_type == 'W' || channel_type == 'w') && (channel <= 128)) // VHT160 support
				ret = snprintf_s(new_cmds, sizeof(new_cmds),
					sizeof(new_cmds) -1, (const char *)cmd_str,
	...
	ret = strncpy_s(cmd_str, sizeof(new_cmds), new_cmds, sizeof(new_cmds) - 1);
	if (ret != 0)
		printk(KERN_ERROR "convert_mode_cmd_str strncpy_s fail");
	return 0;
}
...
static int convert_freq_cmd_str(char *cmd_str, int channel)
{
	...
	ret = snprintf_s(new_cmds, sizeof(new_cmds),
		sizeof(new_cmds) - 1, cmd_str, channel);
	if (ret < 0) {
		printk(KERN_ERROR "convert_freq_cmd_str snprintf_s fail");
		return;
	}
	ret = strncpy_s(cmd_str, sizeof(new_cmds), new_cmds, sizeof(new_cmds) -1);
	if (ret != 0)
		printk(KERN_ERROR "convert_freq_cmd_str strncpy_s fail");
	return 0;
}
...
static int convert_sniffer_cmd_str(char *cmd_str, const char *mac_filter)
{
	...
	int ret;
	ret = snprintf_s(new_cmds, sizeof(new_cmds), sizeof(new_cmds) - 1,
		cmd_str, mac_filter[0], mac_filter[1], mac_filter[2],
		mac_filter[3], mac_filter[4], mac_filter[5]);
	if (ret < 0) {
		printk(KERN_ERROR "convert_sniffer_cmd_str snprintf_s fail");
		return;
	}
	ret = strncpy_s(cmd_str, sizeof(new_cmds),
		new_cmds, sizeof(new_cmds) - 1);
	if (ret != 0)
		printk(KERN_ERROR "convert_sniffer_cmd_str strncpy_s fail");
	return 0;
}
