...
struct smartpa_afe_interface {
	...
	int (*voice_set_customize_param)(uint32_t module_id, uint32_t param_id,
															uint32_t param_size, uint8_t *param_data);
	int (*voice_get_customize_param)(uint32_t module_id, uint32_t param_id,
															uint32_t param_size, uint8_t *param_data);
...
static inline void adsp_misc_unmap_memory(void)
{
	return;
}
#endif
